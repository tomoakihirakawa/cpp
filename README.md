...
$`\phi`$と$`\phi_n`$に関する BIE は，

![equation](<https://latex.codecogs.com/svg.image?&space;\alpha&space;({\bf{a}})\phi&space;({\bf{a}})&space;=&space;\iint_\Gamma&space;{\left{&space;{G({\bf{x}},{\bf{a}})\nabla&space;\phi&space;({\bf{x}})&space;-&space;\phi&space;({\bf{x}})\nabla&space;G({\bf{x}},{\bf{a}})}&space;\right}&space;\cdot&space;{\bf{n}}({\bf{x}})dS}\quad\text{on}\quad{\bf&space;x}&space;\in&space;\Gamma(t).>)
