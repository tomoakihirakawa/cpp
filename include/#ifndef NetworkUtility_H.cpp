...
// Tddd AreaWeightedSmoothingVector(const netPp p) {
//    Tddd ret = {0., 0., 0.}, V = {0., 0, 0.};
//    T3Tddd t3tdd;
//    double Wtot = 0., W;
//    for (const auto &f : p->getFaces()) {
//       t3tdd = ToX(f);
//       // auto [X0, X1, X2] = t3tdd;
//       // W = std::clamp(log10(Circumradius(t3tdd) / Inradius(t3tdd) - 1.), 0., 10.);
//       // W = log2(Circumradius(t3tdd) / Inradius(t3tdd) - 1.);
//       W = TriangleArea(t3tdd) + std::clamp(log10(Circumradius(t3tdd) / Inradius(t3tdd) - 1.), 0., 10.);
//       Wtot += W;
//       // auto X = Norm(X2 - X1) * Normalize(Chop(X0 - X1, X2 - X1)) * sin(M_PI / 3.) + (X2 + X1) / 2.;
//       // auto X = Centroid(t3tdd);
//       V += W * (Incenter(t3tdd) - p->X);
//    }
//    return V / Wtot;
// };
...
//          if (!isFlat(faces[i]->normal, faces[j]->normal, 1E-3 * M_PI / 180.))
//             VECTOR = Projection(VECTOR, Cross(faces[i]->normal, faces[j]->normal));
//    return VECTOR;
// };
Tddd exact_along_surface(const networkPoint *const p, Tddd VECTOR) {
   auto faces = ToVector(p->getFaces());
...
bool isAllFlat(auto p, Tddd V) {
   bool allflat = true;
   double ratio;
   for (const auto &f : p->getFaces()) {
      auto [p0, p1, p2] = f->getPoints(p);
      if (!isFlat(TriangleNormal(T3Tddd{ToX(p0), ToX(p1), ToX(p2)}),
                  TriangleNormal(T3Tddd{ToX(p0) + V, ToX(p1), ToX(p2)}),
                  1E-3 * M_PI / 180.)) {
         allflat = false;
         break;
      }
...
void AreaWeightedSmoothingPreserveShape(netPp p, const double lim_rad = 1E-10) {
   ...
   try {
      if (!isEdgePoint(p)) {
         auto V = exact_along_surface(p, a * AreaWeightedSmoothingVector(p));
         if (isAllFlat(p, V))
            p->setX(ToX(p) + V);
      }
   } catch (std::exception &e) {
...
void AreaWeightedSmoothingPreserveShape(const V_netPp &ps /*copy*/, const double lim_rad = 1E-10) {
   for (const auto &p : ps) AreaWeightedSmoothingPreserveShape(p, lim_rad);
};
...
void LaplacianSmoothingPreserveShape(netPp p, const double lim_rad = 1E-10) {
   ...
   try {
      if (!isEdgePoint(p)) {
         auto V = exact_along_surface(p, a * Mean(extractXtuple(p->getNeighbors())) - ToX(p));
         if (isAllFlat(p, V))
            p->setX(ToX(p) + V);
      }
   } catch (std::exception &e) {
...
void LaplacianSmoothingPreserveShape(const V_netPp &ps /*copy*/, const double lim_rad = 1E-10) {
   for (const auto &p : ps) LaplacianSmoothingPreserveShape(p, lim_rad);
};
...
void Divide(const auto &uo_lines, const std::function<bool(const networkLine *)> &func) {
   for (const auto &l : uo_lines)
      if (func(l))
         l->divide();
};
