...
public:
  ...
  void set()
  {
    ...
    this->eps = data[5];
    this->theta = data[6];
    this->q = cos(data[6] / 180. * M_PI);
    this->p = sin(data[6] / 180. * M_PI);
    this->B = data[19];
    this->G = data[20];
    this->Qmax = data[21];
    this->kappa = 2. * M_PI / LL;
    this->h = D / kappa; //有次元の水深
    this->w = sqrt(g_accel * kappa / G);
    ...
    do
    {
      ...
      for (int k = 0; k < max; k++)
        for (int j = 2 - k % 2; j < max; j++)
        {
          ...
          tmp1 = akj * cos(k * Y) * cos(j * T);
          tmp2 = akj * sin(k * Y) * sin(j * T);
          tmp3 = akj * cos(k * Y) * sin(j * T);
          Gt += j * hyp * tmp1;
          Gy += -k * hyp * tmp2;
          Gz += hypz * akj * tmp3;
          Gzt += j * hypz * tmp1;
  ...
  double PHI_without_mean_velocity(const double T, const double Y, const double Z)
  {
    ...
    for (auto k = 0; k < max; k++)
      for (auto j = 2 - k % 2; j < max; j++)
      {
        akj = a[j][k];
        alp = sqrt(j * j * p * p + k * k * q * q);
        hyp = cosh(alp * Z) + sinh(alp * Z) * tanh(alp * D);
        ret += akj * hyp * sin(j * T) * cos(k * Y);
      }
    return ret;
  };
  //--------------------------------------------------
  ...
  double PHI_TN(const double T, const double Y, const double Z, const int N)
  {
    ...
    for (auto k = 0; k < max + 1; k++)
      for (auto j = 2 - k % 2; j < max + 1; j++)
      {
        ...
        {
        case 0:
          ret = ret + akj * hyp * sin(j * T) * cos(k * Y);
        case 1:
          ret = ret + j * akj * hyp * cos(j * T) * cos(k * Y);
        case 2:
          ret = ret + -j * j * akj * hyp * sin(j * T) * cos(k * Y);
        case 3:
          ret = ret + -j * j * j * akj * hyp * cos(j * T) * cos(k * Y);
        case 4:
          ret = ret + j * j * j * j * akj * hyp * sin(j * T) * cos(k * Y);
        case 5:
          ret = ret + j * j * j * j * j * akj * hyp * cos(j * T) * cos(k * Y);
        case 6:
          ret = ret + -j * j * j * j * j * j * akj * hyp * sin(j * T) * cos(k * Y);
        case 7:
          ret = ret + -j * j * j * j * j * j * j * akj * hyp * cos(j * T) * cos(k * Y);
        }
      }
    return ret;
  };
