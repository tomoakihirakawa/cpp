...
search_detail*/
	...
	void search(const bool getstartObj = true)
	{
		...
		do
		{
			...
			{
				for (const auto &l : p->networkObject::getLines())
				{
...
public:
	V_netLp intxn;
	searcherIntxn() : searcher<networkFace>(), intxn({}){};
	// searcherIntxn(networkFace *obj) : searcher<networkFace>(obj){};

	bool condEnterLine(const networkFace *f, const netLp l) override
	{
		if (l && l->isIntxn())
		{
			this->intxn.emplace_back(l);
			return false;
		}
		// netFp w;
		// if (l && (w = (*l)(f)) && w->intersectQ())
		// {
		//     this->netObjs__.emplace_back((*l)(f));
		//     return false;
		// }
		return true;
	};
	bool condKeepSearch(const netLp l, const networkFace *f) override
	{
		if (f->intersectQ())
			return false;
		else
	...
	bool condGetObject(const netLp l, const networkFace *f) override
	{
		if (f && f->intersectQ())
			return false;
		else
