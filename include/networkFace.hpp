...
inline std::unordered_set<networkPoint *> networkFace::particlize(const double dx, const V_d &depth_list)
{
	...
	{
		auto [p0_, p1_, p2_] = this->getPointsTuple();
		if (count % 3 == 1)
		{
			p0 = p2_;
			p1 = p0_;
		...
		T3Tddd X0X1X2 = {p0->getXtuple() + d / Dot(p0->getNormalTuple(), this->getNormalTuple()) * p0->getNormalTuple(),
						 p1->getXtuple() + d / Dot(p1->getNormalTuple(), this->getNormalTuple()) * p1->getNormalTuple(),
						 p2->getXtuple() + d / Dot(p2->getNormalTuple(), this->getNormalTuple()) * p2->getNormalTuple()};
		for (const auto [xyz, t0t1] : triangleIntoPoints(X0X1X2, dx))
		{
			auto p = new networkPoint(this->getNetwork(), this->getNetwork(), xyz);
...
VV_netPp networkFace::getPointsCutLines() const
{
	...
	while (x_ing.size() >= 2)
	{
		...
		std::cout << blue << "x_ed " << x_ed << reset << std::endl;
#endif
