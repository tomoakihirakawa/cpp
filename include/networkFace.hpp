...
inline void networkFace::setIntegrationInfo() {
  ...
  auto addIntegrationInfo = [&](networkPoint *const p) {
    ...
    */
    for (const auto &[p, f] : dodecapoint.quadpoint.points_faces)
      temp.push_back({p, f, 0., 0.});
    for (const auto &[p, f] : dodecapoint.quadpoint_l0.points_faces)
      temp.push_back({p, f, 0., 0.});
    for (const auto &[p, f] : dodecapoint.quadpoint_l1.points_faces)
      temp.push_back({p, f, 0., 0.});
    for (const auto &[p, f] : dodecapoint.quadpoint_l2.points_faces)
      temp.push_back({p, f, 0., 0.});
...
inline Tddd networkFace::normalVelocityRigidBody(const Tddd &X) const { return this->normal * Dot(this->normal, this->network->velocityRigidBody(X)); };

// コンストラクタ
inline networkFace::networkFace(Network *network_IN, networkPoint *p0, networkLine *l0, networkPoint *p1, networkLine *l1, networkPoint *p2, networkLine *l2) : Triangle(p0->X, p1->X, p2->X), network(network_IN) {
  try {
...
// コンストラクタ
inline networkFace::networkFace(Network *network_IN, networkPoint *p0, networkPoint *p1, networkPoint *p2) : Triangle(p0->X, p1->X, p2->X), network(network_IN) {
  try {
    this->network->add(this);
...
// コンストラクタ
inline networkFace::networkFace(const netFp f) : Triangle(extractXtuple(f)), network(f->network), Lines(f->Lines), Points(f->Points), PLPLPL(f->PLPLPL) {
  this->network->add(this);
