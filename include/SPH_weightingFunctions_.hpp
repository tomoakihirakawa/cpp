...
#pragma omp single nowait
   {
      PW->total_weight = 0;
      PW->tmp_ViscousAndGravityForce = PW->ViscousAndGravityForce = PW->DUDt_SPH_ = PW->gradP_SPH_ = {0., 0., 0.};
      ...
      auto func = [&](const networkPoint *PF, const Tddd &nearX, const double coef = 1.) {
         if (PF->getNetwork()->isFluid || PF->isFluid) {
            ...
            PW->ViscousAndGravityForce += coef * PF->ViscousAndGravityForce * W;
            PW->tmp_ViscousAndGravityForce += coef * PF->tmp_ViscousAndGravityForce * W;
            if (mirroring) {
   ...
   for (const auto &PW : wall_p) {
      if (isFinite(1. / PW->total_weight, 1E+15) && !PW->isFluid) {
         ...
         PW->ViscousAndGravityForce = PW->ViscousAndGravityForce / PW->total_weight;
         PW->tmp_ViscousAndGravityForce = PW->tmp_ViscousAndGravityForce / PW->total_weight;
      }
   }
};
...
#pragma omp single nowait
      {
         ...
         auto func = [&](const networkPoint *PF, const Tddd &nearX, const bool spp = false) {
            ...
            } else if (Norm(PF->normal_SPH) < 1E-12) {
      ...
      for (const auto &PW : wall_p) {
         {
            if (PW->total_weight > 1E-15) {
               ...
               // PW->setDensityVolume(PW->rho_ / PW->total_weight_, PW->volume_ / PW->total_weight_);
...
#pragma omp single nowait
         {
            obj->BucketPoints.apply(p->X, p->radius_SPH * C, [&](const auto &q) {
               ...
               if (Norm(q->normal_SPH) < 1E-12 && Distance(p, q) < p->radius_SPH) {
                  q->isCaptured_ = true;
                  q->isFluid = true;
               }
            });
      ...
      do {
         ...
         // b$ -------------------------------------------------------------------------- */
         init_Lap_U(net->getPoints());
         init_Lap_U(wall_as_fluid);
...
#pragma omp single nowait
         ...
         init_div_tmpU(net->getPoints());
         init_div_tmpU(wall_as_fluid);
...
#pragma omp single nowait
            {
                  ...
                  if (q->isCaptured) {
                     ...
                     auto func = [&](const auto &qX, const double coef = 1.) {
                        ...
                        if (Distance(p, qX) > 1E-12) {
                           ...
                           Aij = 2. * q->mass / p->rho * share;
#else
                           // Nomeritae
                           // Shao and Lo
                           // auto Aij = q->mass * 8. / std::pow(q->rho + p->rho, 2) * share;, which is the same as
                           Aij = 2. * q->mass / std::pow((q->rho + p->rho) / 2., 2) * share;
#endif
...
#if defined(Morikawa2019)
               auto b = _WATER_DENSITY_ * p->div_tmpU / dt;
#else
               auto b = p->div_tmpU / dt;
#endif
               ...
               if (!isFinite(p->p_SPH_) || !isFinite(v) || !isFinite(sum_Aij_Pij)) {
                  ...
                  std::cout << "p->div_tmpU = " << p->div_tmpU << std::endl;
            ...
            for (const auto &[p, newton, v, _] : P2Newton) {
               ...
               // if (p->isSurface && USE_SPP_Pressure){
...
#pragma omp single nowait
         {
            ...
            auto FUNC = [&](const auto &q) {
               ...
               if (q->getNetwork()->isFluid || q->isFluid)
