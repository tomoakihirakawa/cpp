...
class networkFace : public Triangle {
  ...
  public:
   ...
   void reverseNormal() {
      ...
      this->setGeometricProperties(ToX(this->setPoints(this->Lines)));  // setBoundsは，setPointsFromCurrentLines()を実行する．
   };
   /* ------------------------------------------------------ */
...
inline netFp networkFace::divide(netLp DivL /*this*/, netLp newDivL, netLp newMidL, int type) {
  ...
  //    -------DivL-------         --------DivL---------
   //
   if (oldF->Switch(fL, newMidL)) {  // oldFのfLをnewMidLに繋ぎ直し，fLはnewFと繋げる
      std::stringstream ss;
