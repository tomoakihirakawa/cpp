...
#pragma omp single nowait
   {
      ...
      auto func = [&](const networkPoint *p_fluid, const Tddd &whereToSet, const bool spp = false) {
         accel = {0., 0., 0.};
         mirrored_X = Mirror(whereToSet, mirror_wall, p_wall->normal_SPH);
         // add = do_add ? (p_fluid->rho * Dot(p_fluid->DUDt_SPH - accel, -2 * p_wall->normal_SPH)) : 0;
         add = do_add ? (p_fluid->rho * Dot(p_fluid->DUDt_SPH - accel, mirrored_X - whereToSet)) : 0;
         if (spp) {
            p_wall->total_weight += (weight = p_fluid->volume * std::pow(w_Bspline(Norm(whereToSet - Xi), p_wall->radius_SPH * _CSML_), POWER));
            ...
            p_wall->total_weight += (weight = p_fluid->volume * std::pow(w_Bspline(Norm(whereToSet - Xi), p_wall->radius_SPH * _CSML_), POWER));
            p_wall->p_SPH_ += (p_fluid->p_SPH_SPP + add) * weight;
            p_wall->total_weight += (weight = p_fluid->volume * std::pow(w_Bspline(Norm(mirrored_X - Xi), p_wall->radius_SPH * _CSML_), POWER));
...
void developByEISPH(Network *net,
   ...
   try {
      ...
      double dt = max_dt;
      auto C_CFL_velocity = 0.025;  // dt = C_CFL_velocity*h/Max(U)
...
#pragma omp single nowait
            {
               ...
               for (const auto &[obj, poly] : RigidBodyObject)
                  obj->BucketPoints.apply(p->X, p->radius_SPH, [&](const auto &q) {
                     p->interpolated_normal_SPH_original -= 0.5 * grad_w_Bspline(p->X, q->X, p->radius_SPH);
                     p->interpolated_normal_SPH_original_all -= grad_w_Bspline(p->X, q->X, p->radius_SPH);
                  });
...
#pragma omp single nowait
         ...
         // b! ========================================================================== */
         {
            ...
            // setWallPressure(net, wall_p, RigidBodyObject, false);
            DebugPrint("流体粒子をもとに壁粒子の圧力の計算", Magenta);
            // setWallPressure(net, wall_p, RigidBodyObject, false);
            // setWallPressure(net, wall_p, RigidBodyObject, true);  // 内部では，次回利用する壁粒子の数を少し広くしている(p->isCapture)．
               ...
               while (isReflect) {
                  ...
                  if (p_wall) {
                     ...
                     if (particle_spacing > Distance(p_wall, X))
                        if (Dot(p_wall->X + p_wall->normal_SPH + n * (particle_spacing * (1 / 2. - 1E-5)) - X, n) > 0) {
...
#pragma omp single nowait
            ...
            setWallPressure(net, wall_p, RigidBodyObject, true);
