...
//    if (s < 1.) {
//       if (s < 1 / 3.)
//          return (std::pow(1 - s, 5) - 6. * std::pow(2 / 3. - s, 5) + 15 * std::pow(1 / 3. - s, 5)) * 2187. / (40. * M_PI * h * h * h);
//       else if (s < 2 / 3.)
//          return (std::pow(1 - s, 5) - 6. * std::pow(2 / 3. - s, 5)) * 2187. / (40. * M_PI * h * h * h);
//       else
//          return (std::pow(1 - s, 5)) * 2187. / (40. * M_PI * h * h * h);
//    } else
//       return 0.;
// };
double w_Bspline5(double s/*put in r*/, const double &h) {
   // 上で定義したw_Bspline5と同じ
   if ((s /= h) < 1.) {
      if (s < 1 / 3.)
         return (std::pow(1 - s, 5) - 6. * std::pow(2 / 3. - s, 5) + 15 * std::pow(1 / 3. - s, 5)) * 2187. / (40. * M_PI * h * h * h);
   ...
   if (s < 1.) {
      if (s < 1 / 3.)
         return (xi - xj) / (r * h) * (-5 * std::pow(1. - s, 4) + 30. * std::pow(2 / 3. - s, 4) - 75 * std::pow(1 / 3. - s, 4)) * 2187. / (40. * M_PI * h * h * h);
      else if (s < 2 / 3.)
         return (xi - xj) / (r * h) * (-5 * std::pow(1. - s, 4) + 30. * std::pow(2 / 3. - s, 4)) * 2187. / (40. * M_PI * h * h * h);
      else
...
double w_Bspline3(const double &r, const double &h) {
   ...
   if (s < 1.) {
      if (s < 0.5)
         ...
         return 2. * std::pow(1 - s, 3) / (M_PI * h * h * h);
   } else
      return 0.;
};
double dwdq_Bspline3(const double &r, const double &h) {
   double s = r / h;
   if (s < 1.) {
      if (s < 0.5)
         return (-12. + 18. * s) * s / (M_PI * h * h * h);
      else
         return -6. * std::pow(1 - s, 2) / (M_PI * h * h * h);
   } else
      return 0.;
};
Tddd grad_w_Bspline3(const Tddd &xi, const Tddd &xj, const double h) {
   ...
   auto r = Norm(xi - xj);
   return (xi - xj) / r / h * dwdq_Bspline3(r, h);
};
/* -------------------------------------------------------------------------- */
