...
//
double div_tmp_U(const std::unordered_set<networkPoint *> &ps, const networkPoint *const a, const double h, const double dt) {
   /*
   depend on tmp_U_SPH
   */
   double ret = 0;
...
//
double div_tmp_U(const std::unordered_set<networkPoint *> &ps, const networkPoint *const a, const double h, const int order = 3) {
   // double ret = 0;
   // for (const auto &p : ps)
   //     if (p != a)
   //         ret += p->volume *
   //                Dot(p->tmp_U_SPH, grad_kernel_Bspline(ToX(p), ToX(a), h));
   // return ret;
   /* ------------------------------------------------------ */
   //* これはシンプルに勾配計算の改良版である．
   double ret = 0;
...
/* -------------------------------------------------------------------------- */
Tddd viscous_Term_ShaoAndLo2003(const networkPoint *a, const networkPoint *p, double h) {
...
Tddd viscous_Term_ShaoAndLo2003(const networkPoint *a, const networkPoint *p, double h, const Tddd &X) {
   auto Uij = a->U_SPH - p->U_SPH;
   auto Xij = ToX(a) - X;
   auto nu = p->mu_SPH / p->density + a->mu_SPH / a->density;
   return (p->mass * 8 * nu * Dot(Uij, Xij) * grad_kernel_Bspline(ToX(a), X, h)) / ((p->density + a->density) * Dot(Xij, Xij) + std::pow(1E-4 * h, 2));
};
Tddd viscous_Term_ShaoAndLo2003(const networkPoint *a, const networkPoint *p, double h, const Tddd &X, const Tddd &U) {
   auto Uij = a->U_SPH - U;
   auto Xij = ToX(a) - X;
   auto nu = p->mu_SPH / p->density + a->mu_SPH / a->density;
   return (p->mass * 8 * nu * Dot(Uij, Xij) * grad_kernel_Bspline(ToX(a), X, h)) / ((p->density + a->density) * Dot(Xij, Xij) + std::pow(1E-4 * h, 2));
...
Tddd laplacian_U_ShaoAndLo2003_polygon_boundary(const std::unordered_set<networkPoint *> &ps,
   ...
   for (const auto &p : ps) {
      if (p != a)
         ret += viscous_Term_ShaoAndLo2003(a, p, h);
#if defined(use_space_potential_particle)
      if (p->isSurface && p == a) {
         tmp = viscous_Term_ShaoAndLo2003(a, p, h, ToX(p) + getVectorToSPP(p));
         if (isFinite(tmp))
            ret += tmp;
      }
#endif
...
#if defined(use_space_potential_particle)
         if (p->isSurface) {
            tmp = viscous_Term_ShaoAndLo2003(a, p, h, Y + Reflect(getVectorToSPP(p), n), U);
            if (isFinite(tmp))
               ret += tmp;
         }
#endif
         ret += viscous_Term_ShaoAndLo2003(a, p, h, Y, U);
      }
      // b$ ------------------------------------------------------ */
   }
...
Tddd laplacian_U_ShaoAndLo2003(const std::unordered_set<networkPoint *> &ps, const networkPoint *const a, const double h) {
   /*
   depend on U_SPH
   */
   Tddd ret = {0, 0, 0};
   for (const auto &b : ps)
      if (b != a)
         ret += viscous_Term_ShaoAndLo2003(a, b, h);
   return ret / (a->mu_SPH / a->density);
};
...
void developByEISPH(Network *net,
                    ...
                    double &real_time, const double C_SML, const double particle_spacing, const double max_dt) {
   try {
      ...
      Print("バケットの生成を使って，p->radius_SPHの範囲だけ点を取得", Green);
...
#pragma omp single nowait
      {
         for (const auto &obj : Join(RigidBodyObject, FluidObject))
            p->addContactPoints(obj->getBucketPoints(), ToX(p), p->radius_SPH * 1.1);
      }
      std::cout << green << "Elapsed time: " << Red << watch() << colorOff << " s\n";
...
#pragma omp single nowait
         {
            double r = (p->radius_SPH / C_SML) * 3;  //チェック範囲
            if (p->getContactPoints().size() > 3) {
               p->interpolated_normal_SPH = -normal(p->getContactPoints(), p, r);
               Tddd v;
               for (const auto &q : p->getContactPoints()) {
                  v = ToX(q) - ToX(p);
                  if (Between(Norm(v), {1E-10, r}) && isFlat(p->interpolated_normal_SPH, v, M_PI / 4.)) {
                     p->isSurface = false;
                     break;
                  }
               }
...
#pragma omp single nowait
            {
               ...
               for (const auto &obj : FluidObject)
                  p->lap_U += laplacian_U_ShaoAndLo2003(p->getContactPoints(obj), p, p->radius_SPH);
               for (const auto &obj : RigidBodyObject)
                  p->lap_U += laplacian_U_ShaoAndLo2003(p->getContactPoints(obj), p, p->radius_SPH);
               p->tmp_U_SPH = p->U_SPH + ((p->mu_SPH / p->density) * p->lap_U + _GRAVITY3_) * dt;

               if (!isFinite(p->tmp_U_SPH, 1E+5))
                  throw error_message(__FILE__, __PRETTY_FUNCTION__, __LINE__, "tmp_U_SPH is not a finite vector");
            }
            std::cout << green << "Elapsed time: " << Red << watch() << colorOff << " s\n";
            /* --------------------------------------------------------- */
            Print("U*を使って，仮想的な位置X*へ粒子を移動", Green);
...
#pragma omp single nowait
            {
               p->div_U = 0;
               for (const auto &obj : FluidObject)
                  p->div_U += div_tmp_U(p->getContactPoints(obj), p, p->radius_SPH, dt);
               for (const auto &obj : RigidBodyObject)
                  p->div_U += div_tmp_U(p->getContactPoints(obj), p, p->radius_SPH, dt);

               if (!isFinite(p->div_U, 1E+5))
...
#pragma omp single nowait
            {
               auto prev_P = p->pressure_SPH;
               p->pressure_SPH = 0;
               for (const auto &obj : FluidObject)
                  p->pressure_SPH += pressure_EISPH_Hosseini2007(p->getContactPoints(obj), p, p->radius_SPH, dt);
               for (const auto &obj : RigidBodyObject)
                  p->pressure_SPH += pressure_EISPH_Hosseini2007(p->getContactPoints(obj), p, p->radius_SPH, dt);

               p->DPDt_SPH = (p->pressure_SPH - prev_P) / dt;
               if (!isFinite(p->DPDt_SPH, 1E+5))
                  throw error_message(__FILE__, __PRETTY_FUNCTION__, __LINE__, "DPDt_SPH is not a finite");
            }
            // b! ------------------------------------------------------ */
...
#pragma omp single nowait
            {
               p->gradP_SPH = {0, 0, 0};
               for (const auto &obj : FluidObject)
                  p->gradP_SPH += grad_P_Monaghan1992(p->getContactPoints(obj), p, p->radius_SPH);
               for (const auto &obj : RigidBodyObject)
                  p->gradP_SPH += grad_P_Monaghan1992(p->getContactPoints(obj), p, p->radius_SPH);
               p->DUDt_SPH = -p->gradP_SPH / p->density + (p->mu_SPH / p->density) * p->lap_U + _GRAVITY3_;

               if (!isFinite(p->DUDt_SPH, 1E+5))
         ...
         for (const auto &p : water_points) {
            ...
            //
            if (!isFinite(ToX(p), 1E+5) && !isFinite(p->U_SPH, 1E+5))
