...
void setNormal_Surface(auto &net, const std::unordered_set<networkPoint *> &wall_p,
...
#pragma omp single nowait
      {
         ...
         int count = 0;
         /* ------------------------------------------- */
         PW->total_N = PW->total_weight = PW->p_SPH_ = PW->rho_ = PW->volume_ = 0;
      ...
      for (const auto &PW : wall_p) {
         {
            if (PW->total_weight > 1E-15) {
               PW->p_SPH = PW->p_SPH_;
               PW->setDensityVolume(PW->rho_, PW->volume_);
               ...
               if (do_add) {
                  Tddd accel = {0., 0., 0.};
                  PW->p_SPH = PW->p_SPH_ + PW->rho * Dot(PW->DUDt_SPH_ - accel, -2 * PW->normal_SPH);
               }
            } else
               initialize(PW);
         }
...
void developByEISPH(Network *net, const auto &RigidBodyObjectIN,
                    double &real_time,
                    const double C_SML,
...
#pragma omp single nowait
            ...
            /* -------------------------------------------------------------------------- */
            mapValueOnWall(net, wall_p, RigidBodyObject, {1, 1, 1});
            // setNormal_Surface(net, wall_p, RigidBodyObject);
            //
