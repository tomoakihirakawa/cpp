...
#pragma omp single nowait
   {
      ...
      auto funcOffunc = [&](const networkPoint *q, const Tddd &qX) {
         p_wall->total_weight += (weight = q->volume * w_Bspline(Norm(qX - Xi), p_wall->radius_SPH));
      ...
      net->BucketPoints.apply(Xi, 1.1 * p_wall->radius_SPH, [&](const auto &q) {
         funcOffunc(q, q->X);
         /* ----------------------------------------------------- */
         funcOffunc(q, Mirror(q->X, p_wall->X + p_wall->normal_SPH, p_wall->normal_SPH));
         /* ----------------------------------------------------- */
         if (SPP_pressure && q->isSurface) {
            ...
            if (canSetSPP(net, RigidBodyObject, q, qX, q->radius_SPH / q->C_SML)) {
               funcOffunc(q, qX);
               /* ----------------------------------------------------- */
               funcOffunc(q, Mirror(qX, p_wall->X + p_wall->normal_SPH, p_wall->normal_SPH));
               /* ----------------------------------------------------- */
            }
         }
      });
...
#pragma omp single nowait
   {
      ...
      auto funcOffunc = [&](const networkPoint *q, const Tddd &whereToSet, const double spp_pressure_coef, const bool spp = false) {
         p_wall->total_weight += (weight = q->volume * w_Bspline(Norm(whereToSet - Xi), p_wall->radius_SPH));
         accel = {0., 0., 0.};
         nu = q->mu_SPH / q->rho;
         // add = q->rho * Dot(nu * q->lap_U + _GRAVITY3_ - accel, -2 * p_wall->normal_SPH);

         auto dir = Mirror(q->X, p_wall->X + p_wall->normal_SPH, p_wall->normal_SPH) - q->X;
         add = do_add ? (q->rho * Dot(q->DUDt_SPH - accel, dir)) : 0;
         if (spp)
            p_wall->p_SPH_ += spp_pressure_coef * (q->p_SPH_SPP + add) * weight;
         else
      ...
      net->BucketPoints.apply(Xi, p_wall->radius_SPH, [&](const auto &q) {
         funcOffunc(q, q->X, 1.);
         /* ----------------------------------------------------- */
         funcOffunc(q, Mirror(q->X, p_wall->X + p_wall->normal_SPH, p_wall->normal_SPH), 1.);
         /* ----------------------------------------------------- */
         if (SPP_pressure && q->isSurface) {
            auto qX = (q->X + q->interpolated_normal_SPH * q->radius_SPH / q->C_SML);
            // auto qX = (q->X + q->interpolated_normal_SPH * I * std::pow(q->volume, 1 / 3.));
            if (canSetSPP(net, RigidBodyObject, q, qX, q->radius_SPH / q->C_SML)) {
               funcOffunc(q, qX, SPP_pressure_coef, true);
               /* ----------------------------------------------------- */
               funcOffunc(q, Mirror(qX, p_wall->X + p_wall->normal_SPH, p_wall->normal_SPH), SPP_pressure_coef, true);
            }
         }
      });
   }
...
/* -------------------------------------------------------------------------- */
void setRigidBodyObject_tmpU_and_U_SPH(auto &net, const std::unordered_set<networkPoint *> &wall_p,
                                       const auto &RigidBodyObject) {
   Timer watch;
   for (const auto &p : wall_p)
...
#pragma omp single nowait
   {
      ...
      auto func = [&](const auto &q) {
         auto funcOfunc = [&](const networkPoint *q, const Tddd &qX, const bool reflect) {
            p_wall->total_weight += (weight = q->volume * std::pow(w_Bspline(Norm(qX - Xi), p_wall->radius_SPH), power));
            if (reflect) {
               p_wall->U_SPH_ += Reflect(q->U_SPH, p_wall->normal_SPH) * weight;
               p_wall->tmp_U_SPH_ += Reflect(q->tmp_U_SPH, p_wall->normal_SPH) * weight;
            } else {
         ...
         funcOfunc(q, q->X, false);
         /* ----------------------------------------------------- */
         funcOfunc(q, Mirror(q->X, p_wall->X + p_wall->normal_SPH, p_wall->normal_SPH), true);
         /* ----------------------------------------------------- */
         if (SPP_U && q->isSurface) {
            auto qX = (q->X + q->interpolated_normal_SPH * q->radius_SPH / q->C_SML);
            // auto qX = (q->X + q->interpolated_normal_SPH * I * std::pow(q->volume, 1 / 3.));
            if (canSetSPP(net, RigidBodyObject, q, qX, q->radius_SPH / q->C_SML)) {
               funcOfunc(q, qX, false);
               funcOfunc(q, Mirror(qX, p_wall->X + p_wall->normal_SPH, p_wall->normal_SPH), true);
            }
         }
      };
   ...
   for (const auto &p_wall : wall_p) {
      if (p_wall->total_weight > 1E-15) {
         ...
         // free-slip
         // p_wall->U_SPH = p_wall->U_SPH - 2. * Projection(p_wall->U_SPH, p_wall->normal_SPH);
         // p_wall->tmp_U_SPH = p_wall->tmp_U_SPH - 2. * Projection(p_wall->tmp_U_SPH, p_wall->normal_SPH);
...
#pragma omp single nowait
   {
      ...
      net->BucketPoints.apply(p->X, p->radius_SPH, [&](const auto &q) {
         p->total_weight += (w = w_Bspline(Norm(p->X - q->X), p->radius_SPH));
...
#pragma omp single nowait
         ...
         // b! ========================================================================== */
         {
            ...
            // !                    ∇.∇UとU*を計算                       */
            // ! ------------------------------------------------------ */
            // これが仮に移動した場所で評価されていたのはおかしい
            // 流体が持っている圧力は，仮の位置での圧力であるわけではない．
            setRigidBodyObject_DensityVolume(net, wall_p, RigidBodyObject);
            DebugPrint("流体粒子をもとに壁粒子の圧力の計算", Magenta);
            setRigidBodyObject_Pressure(net, wall_p, RigidBodyObject, true);  // 内部では，次回利用する壁粒子の数を少し広くしている(p->isCapture)．
...
#pragma omp single nowait
            {
               ...
               while (isReflect) {
                  ...
                  if (q) {
                     ...
                     const auto n = Normalize(q->normal_SPH);
                     // if (Dot(q->X + 2 * q->normal_SPH - p->tmp_X, n) > 0) {
                     //    if (Dot(p->tmp_U_SPH, n) < 0) {
                     //       p->DUDt_SPH -= (1 + damping_factor) * Projection(p->tmp_U_SPH, n) / dt;
                     //       p->tmp_U_SPH = p->U_SPH + p->DUDt_SPH * dt;
                     //       // p->tmp_X = p->X + p->tmp_U_SPH * dt;
                     //       p->tmp_X = Mirror(p->tmp_X, q->X + q->normal_SPH + n * particle_spacing / 2., q->normal_SPH);
                     //       // p->DUDt_SPH -= 2. * Projection(p->tmp_U_SPH, n) / dt;
                     //       // p->tmp_U_SPH = p->U_SPH + p->DUDt_SPH * dt;
                     //       // p->tmp_U_SPH *= damping_factor;
                     //       // p->tmp_X += Projection(q->X + 2 * q->normal_SPH - p->tmp_X, n);
                     //       isReflect = true;
                     //    }
                     // }

                     if (Dot(q->X + q->normal_SPH + n * particle_spacing / 2. - p->tmp_X, n) > 0) {
                        if (Dot(p->tmp_U_SPH, n) < 0) {
                           p->DUDt_SPH -= (1 + damping_factor) * Projection(p->tmp_U_SPH, n) / dt;
                           p->tmp_U_SPH = p->U_SPH + p->DUDt_SPH * dt;
                           // p->tmp_X = Mirror(p->tmp_X, q->X + q->normal_SPH + n * particle_spacing / 2., q->normal_SPH);
                           p->tmp_X = Mirror(p->tmp_X, q->X + q->normal_SPH, q->normal_SPH);
                           isReflect = true;
                        }
                     }
                  }
...
#pragma omp single nowait
            {
               ...
               auto func = [&](const auto &q) {
                  ...
                  if (SPP_U && q->isSurface) {
                     ...
                     if (canSetSPP(net, RigidBodyObject, q, qX, q->radius_SPH / q->C_SML)) {
                        // これは自身をも含むことがあるので，自身のために特別に作る必要はない．
                        if (Between(Distance(p, qX), {1E-8, p->radius_SPH})) {
...
#pragma omp single nowait
            ...
            // setRigidBodyObject_tmpU_and_U_SPH(net, wall_p, RigidBodyObject, false);
            setRigidBodyObject_Pressure(net, wall_p, RigidBodyObject, true);
#pragma omp parallel
            for (const auto &p : net->getPoints())
#pragma omp single nowait
            ...
            while (isReflect) {
               ...
               auto closest = [&]() {
                  ...
                  for (const auto &[obj, poly] : RigidBodyObject) {
                     obj->BucketPoints.apply(X, p->radius_SPH, [&](const auto &q) {
                        auto tmp = Distance(X, q);
                        if (p != q && distance > tmp && particle_spacing > tmp) {
                           distance = tmp;
                           P = q;
                        }
                     });
               ...
               if (q) {
                  ...
                  // if (Dot((q->X + q->normal_SPH + n * particle_spacing / 2.) - X, n) > 0)
                  //    if (Dot(p->U_SPH, n) < 0) {
                  ...
                  const double damping_factor = 0.7;
                  auto n = Normalize(q->normal_SPH);
                  // if (Distance(q, X) < particle_spacing)
                  // auto MarkerX = q->X + q->normal_SPH + n * particle_spacing / 2.;
                  if (Dot(q->X + q->normal_SPH - p->RK_X.getX(p->U_SPH), n) > 0) {
                     if (Dot(p->U_SPH, n) < 0) {
