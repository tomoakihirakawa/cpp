...
#pragma omp single nowait
   {
      ...
      net->BucketPoints.apply(Xi, p_wall->radius_SPH, [&](const auto &q) {
         if (Dot(Normalize(q->X - Xi), Normalize(p_wall->normal_SPH)) > cos(100 * M_PI / 180.) /*この条件は妥当だろうか*/) {
            weight = std::pow(Norm(q->X - Xi) / (q->radius_SPH / q->C_SML), -2);
      ...
      if (count == 0) {
         ...
         p_wall->p_SPH = p_wall->p_SPH_ / sum_weight;
   ...
   for (auto i = 0; i < 1; ++i)
#else
   for (auto i = 1; i < OurWall_LoopNumber; ++i)
#endif
   {
#pragma omp parallel
      for (const auto &p_wall : wall_p)
#pragma omp single nowait
      ...
      for (const auto &p_wall : wall_p)
         if (i == 0) {
            ...
            p_wall->setDensity(p_wall->rho * a + p_wall->rho_ * (1 - a));
         }
   }
...
#pragma omp single nowait
   {
      ...
      if (count == 0) {
         p_wall->U_SPH = p_wall->U_SPH_ = {0., 0., 0.};
         p_wall->tmp_U_SPH = p_wall->tmp_U_SPH_ = {0., 0., 0.};
      } else {
         p_wall->U_SPH = p_wall->U_SPH_ /= sum_weight;
         p_wall->tmp_U_SPH = p_wall->tmp_U_SPH_ /= sum_weight;
      }
   }
/* -------------------------------------------------------------------------- */
#if defined(Morikawa2021) || defined(Morikawa2021_IDW)
...
void developByEISPH(Network *net,
   ...
   try {
      ...
      for (const auto &p : net->getPoints()) {
         // 速度に関するCFL条件
         net->BucketPoints.apply(p->X, p->radius_SPH / p->C_SML, [&](const auto &q) {
            if (p != q) {
               double max_dt_vel = C_CFL_velocity * Distance(p, q) / Norm(p->U_SPH);
         ...
         net->BucketPoints.apply(p->X, p->radius_SPH / p->C_SML, [&](const auto &q) {
            if (p != q) {
               double max_dt_acc = C_CFL_accel * std::sqrt(Distance(p, q) / Norm(p->DUDt_SPH));
...
#pragma omp single nowait
         {
            ...
            for (const auto &[obj, poly] : RigidBodyObject)
               obj->BucketPoints.apply(p->X, p->radius_SPH, [&](const auto &q) {
                  p->interpolated_normal_SPH -= 0.8 * grad_w_Bspline(p->X, q->X, p->radius_SPH);
               });
            ...
            bool found = false;
            int c = 0;
            net->BucketPoints.apply(p->X, (p->radius_SPH / C_SML) * 2.5, [&](const auto &q) {
               if (Between(Distance(p, q), {1E-8, p->radius_SPH})) {
                  c++;
            ...
            int count = c;
            for (const auto &[obj, poly] : RigidBodyObject)
               obj->BucketPoints.apply(p->X, (p->radius_SPH / C_SML) * 2.5, [&](const auto &q) {
                  if (Between(Distance(p, q), {1E-8, p->radius_SPH})) {
                     c++;
                     if (found || (isFlat(p->interpolated_normal_SPH, q->X - p->X, M_PI / 5.))) {
                        found = true;
                     }
                  }
               });
...
#pragma omp single nowait
            {
               ...
               auto func = [&](const auto &q) {
                  ...
                  //% ------------------------------------------------------------------------ */
                  if (q->isSurface) {
                     auto qX = (q->X + q->interpolated_normal_SPH * particle_spacing);
                     ...
                     //
                     qX = (q->X + q->interpolated_normal_SPH * particle_spacing * 2);
                     if (Between(Distance(p, qX), {1E-8, p->radius_SPH})) {
                        Xij = p->X - qX;
                        Aij = q->mass * 8. / std::pow(q->rho + p->rho, 2) * Dot(Xij, grad_w_Bspline(p->X, qX, p->radius_SPH)) / Dot(Xij, Xij);
                        sum_Aij += Aij;
               ...
               // if (p->isSurface) {
               //    p->p_SPH_ = (p->div_U / dt + sum_Aij_Pj) / sum_Aij / 2.;
               // } else
               if (count) {
               ...
               for (const auto &[obj, poly] : RigidBodyObject) {
                  ...
                  if (Dot(p->U_SPH, n) < 0) {
                     ...
                     if (n_distance < particle_spacing) {
                        double tmp = (1. - n_distance / particle_spacing);
                        // if (tmp > 1.) {
                        //    tmp = 1;
                        //    p->DUDt_SPH = p->DUDt_SPH - Dot(p->DUDt_SPH, n) + Ureflect / dt * tmp;
                        // } else
                        //    p->DUDt_SPH += Ureflect / dt * tmp;
