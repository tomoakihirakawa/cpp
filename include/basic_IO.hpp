...
template <typename STREAM>
STREAM &operator<<(STREAM &stream, const std::vector<std::complex<double>> &v_a_ib) {
   stream << "{";
   if (!v_a_ib.empty()) {
...
template <typename STREAM>
STREAM &operator<<(STREAM &stream, const std::vector<std::vector<std::complex<double>>> &vv_a_ib) {
   stream << "{";
   if (!vv_a_ib.empty()) {
