...
void networkPoint::calculateBufferPotentialsOnClungSurface()
{
	...
	{
		auto [p0, p1, p2] = face->getPointsTuple();
...
inline std::vector<std::tuple<networkFace *, Tddd>> networkPoint::getContactFacesX() const
{
	...
	{
		auto intxn = IntersectionSphereTriangle_(this->getXtuple(), this->radius, f->getXVertices());
...
inline std::vector<std::tuple<networkFace *, Tddd>> networkPoint::getContactFacesXCloser() const
{
	...
	else
	{
		...
		std::sort(c_faces_sorted.begin(), c_faces_sorted.end(), [X](auto a, auto b)
				  { return isFinite(Norm(std::get<1>(a) - X)) &&
	...
	*/
	if (this->Lines.empty())
	{
		...
		for (const auto &f : B.getAll())
		{
			...
			auto intxn = IntersectionSphereTriangle_(this->getXtuple(), 3. * this->radius, f->getXVertices());
	...
	else
	{
		...
		for (const auto &f : faces)
		{
			if (include_self_network || !(f->getNetwork() == this->getNetwork()))
			{
				...
				if (intxn.isIntersecting)
				{
					...
					{
						// if (Dot(f->getNormalTuple(), this->getNormalTuple()) <= 0)
						// if (MyVectorAngle(f->getNormalTuple(), -this->getNormalTuple()) / M_PI * 180. <= 135. ||
		...
		for (const auto &[f, intxnX_r] : tmpContactFaces)
		{
			...
			*/
			...
			// }
			// if (!shadowed)
			// if (Dot(f->getNormalTuple(), this->getNormalTuple() /* <--メッシュの場合は法線方向を向き合いの判定に利用*/) <= 0.)
			if (this->isThereAnyFacingFace(f, M_PI / 180. * 20.))
				faces_sort.push_back({f, Norm(std::get<1>(intxnX_r))});
		}
		std::sort(faces_sort.begin(), faces_sort.end(), [](const auto &a, const auto &b)
				  { return Norm(std::get<1>(a)) - Norm(std::get<1>(b)) < 1E-20; });
		...
		for (const auto &f : faces_sort)
		{
			...
			for (const auto &n : face_normal_vectors)
				if (isFlat(std::get<0>(f)->getNormalTuple(), n, M_PI / 180))
					duplication = true;
			if (!duplication)
			{
				this->ContactFaces.emplace(std::get<0>(f));
				face_normal_vectors.emplace_back(std::get<0>(f)->getNormalTuple());
		...
		// 		auto intxn = IntersectionSphereTriangle_(X_little_inside, this->radius, f->getXVertices());
		// 		Tddd r = intxn.X - X_little_inside;
		// 		if (intxn.isIntersectingInsideTriangle)
		// 			if (intxn.scale <= 0. /*面の法線方向とは逆向き*/ || Dot(r, f->getNormalTuple()) <= 0.)
		// 			{
		// 				std::get<0>(tmpContactFaces[f]) = intxn.X;
		// 				std::get<1>(tmpContactFaces[f]) = r;
		// 			}
...
inline Tddd networkPoint::getNormalSplineKernelAveraged() const
{
	...
	for (const auto &f : this->getFaces())
		normal += kernel_Bspline5(Norm(f->getXtuple() - this->getXtuple()), radius) * f->getNormalTuple();
	return Normalize(normal);
};
...
inline Tddd networkPoint::getNormalSubAreaAveraged() const
{
	...
	for (const auto &f : this->getFaces())
		normal += f->getSubArea(this) * f->getNormalTuple();
	return Normalize(normal);
};
...
inline Tddd networkPoint::getNormalDirichletInscribedCircleAreaAveraged() const
{
	...
	for (const auto &f : this->getFaces())
		if (f->Dirichlet)
			normal += f->getInscribedCircleArea() * f->getNormalTuple();
	return Normalize(normal);
};
...
inline Tddd networkPoint::getNormalOptimum() const
{
	...
	for (const auto &f : this->getFaces())
		unit_normals.emplace_back(f->getNormalTuple());
