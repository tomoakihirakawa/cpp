...
inline std::unordered_set<networkLine *> networkPoint::getLinesOppsoite() const { return TakeExcept(getLinesAround(), this->Lines); };
...
inline std::vector<std::tuple<networkFace *, Tddd>> networkPoint::getContactFacesXCloser() const {
  ...
  if (!c_faces_sorted.empty())
    std::sort(c_faces_sorted.begin(), c_faces_sorted.end(), [&](auto a, auto b) { return isFinite(Norm(std::get<1>(a) - this->X)) && isFinite(Norm(std::get<1>(b) - this->X)) && (Norm(std::get<1>(a) - this->X) - Norm(std::get<1>(b) - this->X)) < 1E-20; });
...
bool isInContact(const networkPoint *p, const networkFace *f_normal, const networkFace *f_target) { return isInContact(p->X, f_normal->normal, ToX(f_target), p); };
...
inline void networkPoint::addContactFaces(const std::vector<Network *> &objects, bool include_self_network = true) {
  ...
  */
  ...
  auto angle = [&](double distance) {
    auto max_deg = 80.;
    auto min_deg = 30.;
    auto r = distance / this->contact_range;
    ...
    if (F_cX_flag[i] == true)
      for (auto j = i + 1; j < F_cX.size(); ++j)
        if (F_cX_flag[j] == true) {
          ...
          // !向かい合っているものだけを残す場合
          if (std::ranges::none_of(surfaces, [&](const auto &f) { return isFlat(fi->normal, -f->normal, angle(distance)); }))
            F_cX_flag[i] = false;
        }
  }

  //! b 3. フラグが立っているものだけを保存する．ContactFacesには，最も近い接触位置が順番に格納され，同方向の面は存在しない．
  const int max_contact_faces = 10; // 別に多くても構わない
...
std::vector<networkFace *> selectionOfFaces(const networkPoint *const p, const std::unordered_set<networkFace *> &contactFacesIN, const int num, const bool delete_same_direction = true) {
  /*
  ...
  if (!f_dist_sort.empty()) {
    for (const auto &[F, D] : f_dist_sort) {
      if (!delete_same_direction)
        ret.emplace_back(F);
      else if (std::none_of(ret.begin(), ret.end(), [&](const auto &f) { return isFlat(F->normal, -f->normal, M_PI / 180) || isFlat(F->normal, f->normal, M_PI / 180); }))
        ret.emplace_back(F);
      if (ret.size() >= num)
        return ret;
    };
...
/* ------------------------------------------------------ */
inline V_netFp networkPoint::getFaces(networkLine *line) const { return getFacesSort(line); };
...
inline std::vector<networkTetra *> networkPoint::getTetras() const noexcept {
  std::unordered_set<networkTetra *> tetras;
  for (const auto &f : this->Faces)
    for (const auto &t : f->getTetras())
      if (t)
        tetras.emplace(t);
  return {tetras.begin(), tetras.end()};
};
...
inline void networkPoint::setX(const V_d &xyz_IN) { this->setX(Tddd{xyz_IN[0], xyz_IN[1], xyz_IN[2]}); };
...
// コンストラクタ
inline networkPoint::networkPoint(Network *network_IN, const Tddd &xyz_IN, networkLine *xline_IN, networkFace *xface_IN) : network(network_IN), target4FMM(xyz_IN), CoordinateBounds(xyz_IN /*CoordinateBoundsに暗黙に置換される*/), initialX(xyz_IN), signed_distance_vector({0., 0., 0.}), map_Net_ContactPoints({{nullptr, {}}}), particlize_info({nullptr, {nullptr, nullptr, nullptr}, {0., 0.}, 0., 0.}) { this->network->add(this); };
// 逆方向の立体角
...
inline std::vector<networkLine *> networkPoint::getLinesOnSurfaces() const {
  std::unordered_set<networkLine *> lines_on_surfaces;
  for (const auto &f : this->getSurfaces())
    for (const auto &l : f->getLines())
      if (l->Point_A == this || l->Point_B == this)
        lines_on_surfaces.emplace(l);
  return {lines_on_surfaces.begin(), lines_on_surfaces.end()};
};

inline std::vector<networkPoint *> networkPoint::getNeighborPointsOnSurfaces() const {
  auto lines = this->getLinesOnSurfaces();
  std::unordered_set<networkPoint *> points_on_surfaces;
  for (const auto &l : lines)
    if (l->Point_A && l->Point_B) {
      if (l->Point_A == this)
        points_on_surfaces.emplace(l->Point_B);
      else
        points_on_surfaces.emplace(l->Point_A);
    }
  return {points_on_surfaces.begin(), points_on_surfaces.end()};
};