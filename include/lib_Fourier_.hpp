...
template <typename INPUT, typename TREATED, size_t Nk, size_t Mk, size_t Ns, size_t Ms> struct Fourier2DConvolution {
  ...
  // using TREATEDadamarl = DoubleDouble;  //! 2025/08/01の結果　X:double, X:long double, O:DoubleDouble, O:std::float128_t
  ...
  using TREATEDadamarl = TREATED; //! 2025/08/01の結果　X:double, X:long double, O:DoubleDouble, O:std::float128_t

  // using TREATEDconv = std::float128_t;
  // using TREATEDdft = std::float128_t;
  // using TREATEDadamarl = std::float128_t;
  ...
  //   template <typename TYPE, std::size_t Size1, std::size_t Size2> std::complex<TREATEDconv> safe_inner_product(const std::array<std::complex<TYPE>, Size1> &arr1, const std::array<std::complex<TREATEDconv>, Size2> &arr2) {
  //     constexpr std::size_t MinSize = (Size1 < Size2 ? Size1 : Size2);

  //     std::complex<TREATEDconv> sum{};
  //     std::complex<TREATEDconv> compensation{};
  //     for (std::size_t i = 0; i < MinSize; ++i) {
  //       auto term = static_cast<std::complex<TREATEDconv>>(arr1[i]) * arr2[i];
  //       auto y = term - compensation;
  //       auto t = sum + y;
  ...
  //   DoubleDOubleでもかいぜんされなかった．．．．なにかある．原因が．他にもあるのかも．

  //   0．dft,inverseなどに safe_inner_productを正しくいれられているかちぇっく．
  //   1．levelを下げてチェック．
  //   2．以前のバージョンと比較

  template <typename TYPE, std::size_t Size1, std::size_t Size2> std::complex<TREATEDconv> safe_inner_product(const std::array<std::complex<TYPE>, Size1> &arr1, const std::array<std::complex<TREATEDconv>, Size2> &arr2) {
    constexpr std::size_t MinSize = (Size1 < Size2 ? Size1 : Size2);
    std::complex<TREATEDconv> sum = {}, compensation = {}; // 誤差補正項 (c)
    std::complex<TREATEDconv> y, t;
    for (std::size_t i = 0; i < MinSize; ++i) {
      y = (static_cast<std::complex<TREATEDconv>>(arr1[i]) * arr2[i]) - compensation;
      compensation = ((t = sum + y) - sum) - y;
      sum = t;
    }
    return sum;
  }

  template <typename MatrixType1, typename MatrixType2> void adamard_product_add(const std::array<std::array<std::complex<MatrixType1>, cols>, rows> &MatrixDFT1, const std::array<std::array<std::complex<MatrixType2>, cols>, rows> &MatrixDFT2) {
    // for (auto n = 0; n < rows; ++n)
    //   for (auto k = 0; k < cols; ++k) {
    //     this->MatrixDFT[n][k] += static_cast<std::complex<TREATED>>(static_cast<std::complex<TREATED>>(MatrixDFT1[n][k]) * static_cast<std::complex<TREATEDadamarl>>(MatrixDFT2[n][k]));
    //   }

    for (auto n = 0; n < rows; ++n) {
      this->MatrixDFT[n] += safe_inner_product(MatrixDFT1[n], MatrixDFT2[n]);
    }
  };
  ...
  std::array<std::complex<TREATEDconv>, rows> InverseDFT_rows(const std::array<std::complex<TREATEDconv>, rows> &sample) {
    ...
    std::complex<TREATEDconv> sum = {}; // ゼロで正しく初期化する
    for (int n = 0; n < rows; ++n) {
...
#define _SPEEDUP_
  ...
  void InverseDFT() {
    ...
    for (int i = 0; i < rows; ++i) {
      cn2D = InverseDFT_cols(this->MatrixDFT[i]); //! 長さM
