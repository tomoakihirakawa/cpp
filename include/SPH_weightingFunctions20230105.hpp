...
#define Morikawa2021_wall_pressure
   // #define Morikawa2021_IDW

#define REFLECTION

#define OurWall_LoopNumber 2
...
#pragma omp single nowait
         if (Norm(p_wall->normal_SPH) <= 1.1 * (OurWall_LoopNumber - i) / OurWall_LoopNumber * p_wall->radius_SPH) {
            p_wall->p_SPH_ = p_wall->rho_ = p_wall->volume_ = p_wall->total_weight = 0;
            ...
            net->BucketPoints.apply(Xi, p_wall->radius_SPH, [&](const auto &q) {
               ...
               if (SPP_setRigidBodyObject_Pressure && q->isSurface)
                  for (auto I = 1; I <= 1; I++) {
                     ...
                     weight = q->volume * w_Bspline(Norm(qX - Xi), p_wall->radius_SPH);
                     p_wall->total_weight += weight;
                     p_wall->rho_ += q->rho * weight;
                     p_wall->volume_ += q->volume * weight;
                     count++;
               ...
               for (const auto &[obj, poly] : RigidBodyObject)
                  obj->BucketPoints.apply(Xi, p_wall->radius_SPH, [&](const auto &q) {
                     ...
                     if (q == p_wall) {
                        ...
                        p_wall->rho_ += p_wall->rho * weight;
                        p_wall->volume_ += p_wall->volume * weight;
      ...
      for (const auto &p_wall : wall_p)
         if (Norm(p_wall->normal_SPH) <= 1.1 * (OurWall_LoopNumber - i) / OurWall_LoopNumber * p_wall->radius_SPH) {
            if (i > 0) {
               p_wall->setDensityVolume(p_wall->rho_ / (1 - W), p_wall->volume_ / (1 - W));
            } else {
               if (p_wall->total_weight < 1E-15)
                  p_wall->setDensity(_WATER_DENSITY_);
               else
...
#pragma omp single nowait
         if (Norm(p_wall->normal_SPH) <= 1.1 * (OurWall_LoopNumber - i) / OurWall_LoopNumber * p_wall->radius_SPH) {
            p_wall->p_SPH_ = p_wall->rho_ = p_wall->total_weight = 0;
            Tddd Xi = p_wall->X + 2 * p_wall->normal_SPH, accel;
            double weight, nu, add;
            int count = 0;
            net->BucketPoints.apply(Xi, p_wall->radius_SPH, [&](const auto &q) {
      ...
      for (const auto &p_wall : wall_p)
         if (Norm(p_wall->normal_SPH) <= 1.1 * (OurWall_LoopNumber - i) / OurWall_LoopNumber * p_wall->radius_SPH) {
            if (i > 0) {
               p_wall->p_SPH = p_wall->p_SPH_ / (1 - W) * a + p_wall->p_SPH * (1 - a);
               // p_wall->setDensity(p_wall->rho_ / (1 - W) * a + p_wall->rho * (1 - a));
               //
               // p_wall->setDensity(_WATER_DENSITY_);
               // p_wall->p_SPH = p_wall->p_SPH_ * a + p_wall->p_SPH * (1 - a);
               // p_wall->setDensity(p_wall->rho_);
               // p_wall->setDensity(p_wall->rho_ * a + p_wall->rho * (1 - a));
            } else {
               if (p_wall->total_weight < 1E-15) {
                  ...
                  // p_wall->setDensity(p_wall->rho_ / p_wall->total_weight);
                  // p_wall->setDensity(_WATER_DENSITY_);
               }
            }
         }
...
//    /*
//    繰り返して，計算を収束させる理由
//    壁粒子の圧力を，流体領域の鏡写しのようにして一度計算したとする．
//    初回は，近傍の壁粒子の値は初期値のままでゼロとして，自身の値を計算することになる．
//    このようにして計算した壁粒子の値を使って，流体内部の圧力勾配を計算することになるが，
//    圧力勾配を計算する際の壁粒子の圧力は，壁粒子の圧力を見積もった時の，壁粒子の圧力とは違っている．
//    この二つを同じにするためには，壁粒子の圧力を十分に収束させておく必要がある．
//    */
//    Timer watch;
//    for (const auto &p : wall_p) {
//       p->p_SPH_ = p->p_SPH = p->total_weight = 0;
...
#pragma omp single nowait
      ...
      for (const auto &p_wall : wall_p)
         if (Norm(p_wall->normal_SPH) <= 1.1 * (OurWall_LoopNumber - i) / OurWall_LoopNumber * p_wall->radius_SPH) {
            if (i > 0) {
               p_wall->U_SPH = p_wall->U_SPH_ / (1 - W) * a + p_wall->U_SPH * (1 - a);
               p_wall->tmp_U_SPH = p_wall->tmp_U_SPH_ / (1 - W) * a + p_wall->tmp_U_SPH * (1 - a);
...
#pragma omp single nowait
         {
            ...
            p->isSurface = true;
            auto search = [&](const auto &q) { return p != q && (VectorAngle(p->interpolated_normal_SPH_all, q->X - p->X) < M_PI / 5.); };
            if (net->BucketPoints.any_of(p->X, (p->radius_SPH / p->C_SML) * 2.5, search))
               p->isSurface = false;
            else
               for (const auto &[obj, poly] : RigidBodyObject)
                  if (obj->BucketPoints.any_of(p->X, (p->radius_SPH / p->C_SML) * 2.5, search))
                     p->isSurface = false;
         }
...
#pragma omp single nowait
            {
               ...
               auto func = [&](const auto &q) {
                  if (Between(Distance(p, q), {1E-8, p->radius_SPH})) {
                     ...
                     tmp /= (q->rho + p->rho) * std::pow(Distance(p, q), 2) + std::pow(1E-4 * p->radius_SPH, 2);
                     viscous_term += tmp;
                     //
                  ...
                  if (SPP_lap_U && q->isSurface)
                     for (auto I = 1; I <= 1; ++I) {
                        ...
                        if (Between(Distance(p, qX), {1E-8, p->radius_SPH})) {
                           ...
                           tmp /= (q->rho + p->rho) * std::pow(Distance(p, qX), 2) + std::pow(1E-4 * p->radius_SPH, 2);
                           viscous_term += tmp;
                        }
                     }
               };
