...
namespace obj3D
{
	...
	//--------------------------------
	template <class T>
	std::unordered_set<T *> takeIfBoundariesOverlap(const std::unordered_set<T *> &objs, const geometry::CoordinateBounds &bounds)
	{
	...
	template <class T>
	std::vector<T *> takeIfBoundariesOverlap(const std::vector<T *> &objs, const geometry::CoordinateBounds &bounds)
	{
		std::vector<T *> ret;
		for (const auto &obj : objs)
			if (IntersectQ(obj->getBounds(), bounds))
...
struct object3D
{
	object3D(const geometry::CoordinateBounds &bs)
	{
		this->bounds = bs;
	...
	void setBounds(const geometry::CoordinateBounds &bs)
	{
		this->bounds = bs;
		std::get<0>(this->X) = Mean(std::get<0>(bs.bounds));
		std::get<1>(this->X) = Mean(std::get<1>(bs.bounds));
	...
	{
		this->setBounds(geometry::CoordinateBounds(X012));
		this->X = Mean(X012);
	};
};
#endif
