...
#define POWER 1.
#define _CSML_ 1.
#define normalize_threshold 1000000000000.
...
#pragma omp single nowait
   {
      ...
      double weight;
      int count = 0;
      // if (net->BucketPoints.any_of(Xi, p_wall->radius_SPH / 2., [&](const auto &p_fluid) { return p_fluid; }))
      {
   ...
   for (const auto &p_wall : wall_p) {
      {
         if (p_wall->total_weight < 1E-15)
            ...
            // if (p_wall->total_weight < normalize_threshold) {
