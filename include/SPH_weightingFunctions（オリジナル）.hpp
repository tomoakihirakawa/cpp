...
#pragma omp single nowait
      {
         ...
         int count = 0;
         net->BucketPoints.Apply(Xi, p_wall->radius_SPH, [&](const auto &uo_q) {
            for (const auto &q : uo_q) {
#if defined(Morikawa2021_IDW)
               weight = std::pow(Norm(q->X - Xi), -2);
#else
                  weight = q->volume * w_Bspline5(Norm(q->X - Xi), p_wall->radius_SPH);
#endif
         ...
         for (const auto &[obj, poly] : RigidBodyObject) {
            obj->BucketPoints.Apply(Xi, p_wall->radius_SPH, [&](const auto &uo_q) {
               for (const auto &q : uo_q)
                  p_wall->p_SPH_ += q->p_SPH * q->volume * w_Bspline5(Norm(q->X - Xi), p_wall->radius_SPH);
            });
         }
#endif
      }
   ...
   std::cout << Blue << "setRigidBodyObject_Pressure Elapsed time: " << Red << watch() << colorOff << " s\n";
};
// b! -------------------------------------------------------------------------- */
...
#pragma omp single nowait
...
#endif
               // free-slip
               // if (Between(Norm(q->X - Xi), {1E-10, 1E+10})) {
   ...
   std::cout << Blue << "setRigidBodyObject_U_SPH Elapsed time: " << Red << watch() << colorOff << " s\n";
};
/* -------------------------------------------------------------------------- */
/* -------------------------------------------------------------------------- */
/* -------------------------------------------------------------------------- */
...
#pragma omp single nowait
         ...
         Print("流体粒子の法線方向の計算，水面の判定", Green);
         //$ 水粒子のオブジェクト外向き法線方向を計算
#pragma omp parallel
         for (const auto &p : net->getPoints())
#pragma omp single nowait
         {
            ...
            auto search = [&](const auto &q) {
               if (found || (Between(Distance(p, q), {1E-10, r}) && isFlat(p->interpolated_normal_SPH, q->X - p->X, M_PI / 4.)))
                  found = true; };
...
#pragma omp single nowait
            {
               ...
               for (const auto &[obj, poly] : RigidBodyObject)
                  obj->BucketPoints.apply(p->X, p->radius_SPH * C, [&](const auto &q) {
                     if (Between(Distance(p, q), {1E-10, p->radius_SPH * C}))
                        p->interpolated_normal_SPH -= grad_w_Bspline5(p->X, q->X, p->radius_SPH * C);
                  });
               p->interpolated_normal_SPH = Normalize(p->interpolated_normal_SPH);
               if (!isFinite(p->interpolated_normal_SPH))
                  p->interpolated_normal_SPH = {0., 0., 1.};
...
#pragma omp single nowait
         {
            for (const auto &[obj, poly] : RigidBodyObject) {
               Tddd A = p->radius_SPH * C * p->interpolated_normal_SPH + p->X;
               Tddd B = p->X;
               T2Tddd AB = {A, B};
               Tddd vec2Face;
               double distance = 1E+20;
               for (const auto &f : poly->getFaces()) {
                  if (IntersectQ(AB, ToX(f))) {
                     vec2Face = XonTriangle(ToX(f), AB) - p->X;
                     if (distance > Norm(vec2Face))
                        p->normal_SPH = vec2Face;
                  }
               }
            }
         }
...
#pragma omp single nowait
            {
               ...
               auto func = [&](const auto &q) {
                  if (Between(Distance(p, q), {1E-10, p->radius_SPH})) {
                     // p->div_U += q->mass / p->rho * Dot(q->tmp_U_SPH - p->tmp_U_SPH, grad_w_Bspline5(p->X, q->X, p->radius_SPH));
                     // 下の方法の方が安定する
                     qp = q->X - p->X;
...
#pragma omp single nowait
            {
               ...
               net->BucketPoints.apply(p->X, p->radius_SPH, [&](const auto &q) {
                  if (Between(Distance(p, q), {1E-10, p->radius_SPH})) {
                     ...
                     p->gradP_SPH += p->rho * q->mass * p_rho2 * grad_w_Bspline5(p->X, q->X, p->radius_SPH);
                     count++;
                  }
               });
               //
...
#pragma omp single nowait
         {
            for (const auto &[obj, poly] : RigidBodyObject) {
               ...
               if (p->isInsideOfBody = inside) {
                  auto n = Normalize(cell->Interpolate(p->X, cell->vectors));
                  if (Dot(p->U_SPH, n) < 0) {
