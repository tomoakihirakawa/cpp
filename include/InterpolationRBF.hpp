...
サンプル点の平均的な間隔を${s}$とした場合，${\vareps = 1/s}$とパラメタをとるとよい．
...
public:
	...
	void set(const VV_d &P_IN, const VV_d &VV_IN)
	{
		...
		{ return kernel_MQ(x, a, 1. / this->scale); };
		this->grad_phi = [this](const V_d &x, const V_d &a)
		{ return grad_kernel_MQ(x, a, 1. / this->scale); };
		this->laplacian_phi = [this](const V_d &x, const V_d &a)
		{ return laplacian_kernel_MQ(x, a, 1. / this->scale); };
		//!関数を代入していからweight計算
