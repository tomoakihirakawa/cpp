...
private:
    ...
    //         //------------------
    //         if (d == 1)
    //             fs = p->getFaces(); //これだけならgetNormal()と同じ
    //         else
    //         {
...
public:
    ...
    CompGrid() : mandatoryP({}), InnerP({}), CornerP({}), OuterP({}), PointsForBIE({}), InnerOuterP({}), InnerCornerP({}),
                 all_Faces({}), well_Faces({}), ill_Faces({}), routeP({}), xnet(nullptr){};

    CompGrid(const V_Netp &base_net, const VV_d &cardinal, const V_netPp mandatoryP_IN = {})
        : mandatoryP({}), InnerP({}), CornerP({}), OuterP({}), PointsForBIE({}), InnerOuterP({}), InnerCornerP({}),
          all_Faces({}), well_Faces({}), ill_Faces({}), routeP({}), xnet(nullptr)
    {
        ...
        mk_vtu("./vtu/cpg_" + xnet->getName() + "_Faces.vtu", this->xnet->getFaces());
    ...
    void makeXFaces(const V_Netp &base_net, const VV_d &cardinals)
    {
        ...
        try
        {
            ...
            // {
            //     map[n] = n->getFaces();
            //     mk_vtu("./vtu/makeFaces_" + n->getName() + "->Faces" + std::to_string(time_step) + ".vtu", n->getFaces());
            // }
            // std::cin.ignore();
            ////
    ...
    void deleteUnrelatedFaces(const V_Netp &base_nets, const VV_d &cardinals)
    {
        ...
        for (const auto &f : DeleteDuplicates(this->xnet->getFaces()))
