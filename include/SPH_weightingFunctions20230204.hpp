...
#pragma omp single nowait
   {
      ...
      if (net->BucketPoints.any_of(Xi, p_wall->radius_SPH / p_wall->C_SML * 1.1, [&](const auto &p_fluid) { return p_fluid; })) {
         auto func = [&](const networkPoint *p_fluid, const Tddd &whereToSet) {
            p_wall->total_weight += (weight = p_fluid->volume * w_Bspline(Norm(whereToSet - Xi), p_wall->radius_SPH));
            ...
            p_wall->total_weight += (weight = p_fluid->volume * w_Bspline(Norm(mirrored_X - Xi), p_wall->radius_SPH));
            p_wall->rho_ += p_fluid->rho * weight;
            p_wall->volume_ += p_fluid->volume * weight;
            count++;
         };
         ...
         //       p_wall->total_weight += (weight = vol * std::pow(w_Bspline(Norm(q->X - Xi), p_wall->radius_SPH * p_wall->C_SML), POWER));
...
#pragma omp single nowait
   {
      ...
      auto func = [&](const networkPoint *p_fluid, const Tddd &whereToSet, const bool spp = false) {
         ...
         mirrored_X = Mirror(whereToSet, mirror_wall, p_wall->normal_SPH);
         if (spp) {
            p_wall->total_weight += (weight = p_fluid->volume * std::pow(w_Bspline(Norm(whereToSet - Xi), p_wall->radius_SPH * CSML), POW));
            p_wall->DUDt_SPH_ += p_fluid->DUDt_SPH_ * weight;
            p_wall->total_weight += (weight = p_fluid->volume * std::pow(w_Bspline(Norm(mirrored_X - Xi), p_wall->radius_SPH * CSML), POW));
            p_wall->DUDt_SPH_ += p_fluid->DUDt_SPH_ * weight;
         } else {
            p_wall->total_weight += (weight = p_fluid->volume * std::pow(w_Bspline(Norm(whereToSet - Xi), p_wall->radius_SPH * CSML), POW));
            p_wall->DUDt_SPH_ += p_fluid->DUDt_SPH_ * weight;
...
void mapValueOnWall(auto &net,
   ...
   double PO, C_SML = 1.;
   auto set = [&](const bool applyToWall = false) {
#pragma omp parallel
      for (const auto &PW : wall_p)
#pragma omp single nowait
      {
         ...
         int count = 0;
         /* ------------------------------------------- */
         auto func = [&](const networkPoint *PF, const Tddd &nearX) {
            mirrored_X = Mirror(nearX, mirror_wall, PW->normal_SPH);
            PW->total_weight += (W = PF->volume * std::pow(w_Bspline(Norm(nearX - markerX), PW->radius_SPH / PW->C_SML * C_SML), POW));
            PW->p_SPH_ += PF->p_SPH * W;
            PW->DUDt_SPH_ += PF->DUDt_SPH * W;
            PW->rho_ += PF->rho * W;
            PW->U_SPH_ += Reflect(PF->U_SPH, n) * W;
            PW->tmp_U_SPH_ += Reflect(PF->tmp_U_SPH, n) * W;
            PW->volume_ += PF->volume * W;
            //
            PW->total_weight += (W = PF->volume * std::pow(w_Bspline(Norm(mirrored_X - markerX), PW->radius_SPH / PW->C_SML * C_SML), POW));
            PW->p_SPH_ += PF->p_SPH * W;
            PW->DUDt_SPH_ += PF->DUDt_SPH * W;
            PW->rho_ += PF->rho * W;
            PW->U_SPH_ += Reflect(PF->U_SPH, n) * W;
   ...
   for (const auto &PW : wall_p) {
      {
         if (PW->total_weight < 1E-15) {
            ...
            PW->rho = PW->rho_ / PW->total_weight;
            PW->volume = PW->volume_ / PW->total_weight;
   ...
   for (const auto &PW : wall_p) {
      {
         if (PW->total_weight < 1E-15) {
            ...
            PW->p_SPH = PW->p_SPH_;
            PW->rho = PW->rho_;
            PW->volume = PW->volume_;
            PW->DUDt_SPH = PW->DUDt_SPH_;
            PW->U_SPH = PW->U_SPH_;
            PW->tmp_U_SPH = PW->tmp_U_SPH_;
            //
            if (do_add) {
               Tddd accel = {0., 0., 0.};
...
void preSetWallPressure(auto &net, const std::unordered_set<networkPoint *> &wall_p,
                        const auto &RigidBodyObject,
                        const int do_add = 0) {
   Timer watch;
   for (const auto &p : wall_p) {
      p->p_SPH_ = p->p_SPH = p->total_weight = 0;
      p_wall->U_SPH_ = p_wall->tmp_U_SPH_ = p->DUDt_SPH = {0., 0., 0.};
   }
#pragma omp parallel
   for (const auto &p_wall : wall_p)
#pragma omp single nowait
   {
      ...
      p_wall->DUDt_SPH_ = {0., 0., 0.};
      Tddd markerX = p_wall->X + 2 * p_wall->normal_SPH, accel, mirror_wall = p_wall->X + p_wall->normal_SPH, mirrored_X, n = Normalize(p_wall->normal_SPH);
      double weight, nu, add, value;
      int count = 0;
      ...
      auto func = [&](const networkPoint *p_fluid, const Tddd &whereToSet, const bool spp = false) {
         ...
         mirrored_X = Mirror(whereToSet, mirror_wall, p_wall->normal_SPH);
         if (spp) {
            p_wall->total_weight += (weight = p_fluid->volume * std::pow(w_Bspline(Norm(whereToSet - markerX), p_wall->radius_SPH), POW));
            p_wall->p_SPH_ += p_fluid->p_SPH_SPP * weight;
            p_wall->DUDt_SPH_ += p_fluid->DUDt_SPH * weight;
            p_wall->rho_ += p_fluid->rho * weight;
            p_wall->U_SPH_ += p_fluid->U_SPH * weight;
            p_wall->tmp_U_SPH_ += p_fluid->tmp_U_SPH * weight;
            //
            p_wall->total_weight += (weight = p_fluid->volume * std::pow(w_Bspline(Norm(mirrored_X - markerX), p_wall->radius_SPH), POW));
            p_wall->p_SPH_ += p_fluid->p_SPH_SPP * weight;
            p_wall->DUDt_SPH_ += p_fluid->DUDt_SPH * weight;
            p_wall->rho_ += p_fluid->rho * weight;
            p_wall->U_SPH_ += p_fluid->U_SPH * weight;
      ...
      net->BucketPoints.apply(markerX, p_wall->radius_SPH, [&](const auto &p_fluid) {
         func(p_fluid, p_fluid->X);
         if (SPP_pressure && p_fluid->isSurface) {
            auto ps = p_fluid->radius_SPH / p_fluid->C_SML;
            auto whereToSet = (p_fluid->X + p_fluid->interpolated_normal_SPH * ps);
   ...
   for (const auto &p_wall : wall_p) {
      {
         if (p_wall->total_weight < 1E-15) {
            ...
            p_wall->rho = p_wall->rho_ / p_wall->total_weight;
            p_wall->DUDt_SPH = p_wall->DUDt_SPH_ / p_wall->total_weight;
         }
      }
...
void setWallPressure(auto &net, const std::unordered_set<networkPoint *> &wall_p,
   ...
   preSetWallPressure(net, wall_p, RigidBodyObject, do_add);
...
#pragma omp single nowait
   {
      ...
      auto CSML = p_wall->C_SML;
      auto POW = 1.;
      auto func = [&](const networkPoint *p_fluid, const Tddd &p_X, const bool spp = false) {
         accel = {0., 0., 0.};
         mirrored_X = Mirror(p_X, mirror_wall, p_wall->normal_SPH);
         if (spp) {
            p_wall->total_weight += (weight = p_fluid->volume * std::pow(w_Bspline(Norm(p_X - markerX), p_wall->radius_SPH), POW));
            p_wall->p_SPH_ += p_fluid->p_SPH_SPP * weight;
            p_wall->DUDt_SPH_ += p_fluid->DUDt_SPH * weight;
            p_wall->rho_ += p_fluid->rho * weight;

            // p_wall->total_weight += (weight = p_fluid->volume * std::pow(w_Bspline(Norm(mirrored_X - markerX), p_wall->radius_SPH * CSML), POW));
            // p_wall->p_SPH_ += p_fluid->p_SPH_SPP * weight;
            // // p_wall->DUDt_SPH_ += p_fluid->DUDt_SPH_ * weight;
            // p_wall->rho_ += p_fluid->rho * weight;
         } else {
      ...
      for (const auto &[obj, poly] : RigidBodyObject)
         obj->BucketPoints.apply(markerX, p_wall->radius_SPH, [&](const auto &q_wall) {
            func(q_wall, q_wall->X);
         });
   }
   //
   for (const auto &p_wall : wall_p) {
      {
         if (p_wall->total_weight < 1E-15) {
            ...
            Tddd accel = {0., 0., 0.};
            if (do_add)
               p_wall->p_SPH = p_wall->p_SPH_ + p_wall->rho_ * Dot(p_wall->DUDt_SPH_ - accel, -2 * p_wall->normal_SPH);
            else
...
#pragma omp single nowait
   {
      ...
      p_wall->total_weight = 0;
      Tddd markerX = p_wall->X + 2 * p_wall->normal_SPH, mirrored_X, mirror_wall = p_wall->X + p_wall->normal_SPH;
      double weight;
      int count = 0;
      ...
      auto func = [&](const networkPoint *p_fluid, const Tddd &whereToSet) {
         p_wall->total_weight += (weight = p_fluid->volume * w_Bspline(Norm(whereToSet - markerX), p_wall->radius_SPH));
         p_wall->U_SPH_ += p_fluid->U_SPH * weight;
         p_wall->tmp_U_SPH_ += p_fluid->tmp_U_SPH * weight;
         //
         mirrored_X = Mirror(whereToSet, mirror_wall, p_wall->normal_SPH);
         p_wall->total_weight += (weight = p_fluid->volume * w_Bspline(Norm(mirrored_X - markerX), p_wall->radius_SPH));
...
#pragma omp single nowait
            {
               ...
               auto func = [&](const auto &q) {
                  ...
                  if (SPP_pressure && q->isSurface) {
                     qX = q->X + q->interpolated_normal_SPH * q->radius_SPH / q->C_SML;
                     if (canSetSPP(net, RigidBodyObject, q, qX, q->radius_SPH / q->C_SML) && Between(Distance(p, qX), {1E-8, p->radius_SPH})) {
                        p_rho2 = q->p_SPH_SPP / std::pow(q->rho, 2) + p->p_SPH / std::pow(p->rho, 2);
