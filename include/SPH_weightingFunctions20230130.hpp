...
#pragma omp single nowait
   {
      ...
      auto func = [&](const networkPoint *p_fluid, const Tddd &whereToSet, const bool spp = false) {
         ...
         add = do_add ? (p_fluid->rho * Dot(p_fluid->DUDt_SPH_ - accel, mirrored_X - whereToSet)) : 0;
         if (spp) {
            p_wall->total_weight += (weight = p_fluid->volume * std::pow(w_Bspline(Norm(whereToSet - Xi), p_wall->radius_SPH * _CSML_), POWER));
   ...
   for (const auto &p_wall : wall_p) {
      {
         if (p_wall->total_weight < 1E-15) {
            ...
            if (do_add == 1)
               p_wall->p_SPH = p_wall->p_SPH_ + p_wall->rho * Dot(p_wall->DUDt_SPH_ - accel, p_wall->X - mirrored_X);
            else if (do_add == 2)
               p_wall->p_SPH = p_wall->p_SPH_ + p_wall->rho * Dot(_GRAVITY3_ - accel, p_wall->X - mirrored_X);
            else
