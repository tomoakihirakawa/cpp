...
std::vector<std::vector<std::complex<double>>> operator*(std::vector<std::vector<std::complex<double>>> a,
...
std::array<std::array<std::complex<double>, M>, N> DFT(const std::array<std::array<T, M>, N>& sample2D) {
   //! sample can be real or complex
...
auto make_inverse_dft_polar_table() {
...
template <typename T, size_t N, size_t M>
std::array<std::array<std::complex<double>, M>, N> InverseDFT(const std::array<std::array<T, M>, N>& sample2D) {
   std::array<std::array<std::complex<double>, M>, N> result;
   std::complex<double> sum = 0;
   double cx, cy;
   std::array<std::array<std::complex<double>, M>, N> cn2D;
...
struct Fourier2D {
   ...
   void add(const MatrixLike& input, const std::array<bool, 2>& reverse = {false, false}) {
   ...
   template <typename MatrixLike>
   void add(const MatrixLike& input, const std::array<bool, 2>& reverse, const MatrixLike& input2) {
      auto MatrixPaddedDFT = DFT(fill(input, reverse));
   ...
   template <typename MatrixLike>
   void add(const MatrixLike& input, const std::array<bool, 2>& reverse, const Fourier2D<T>& inputDFT2) {
   ...
   template <typename MatrixLike>
   void add_multipy(const MatrixLike& input, const std::array<bool, 2>& reverse, const MatrixLike& input2, double a) {
      auto MatrixPaddedDFT = DFT(fill(input, reverse));
      auto MatrixPaddedDFT2 = DFT(fill(input2));
      for (std::size_t i = 0; i < rows; ++i)
         for (std::size_t j = 0; j < cols; ++j) {
   ...
   template <typename MatrixLike>
   std::vector<std::vector<T>> fill(const MatrixLike& input) {
   ...
   template <typename MatrixLike>
   std::vector<std::vector<T>> fill(const MatrixLike& input, const std::array<bool, 2>& reverse) {
      const auto si = input.size();
      const auto sj = input[0].size();
      std::vector<std::vector<T>> fillingMatrix(std::vector<std::vector<T>>(this->rows, std::vector<T>(this->cols, 0)));
...
struct Quadruple {
   ...
   // 減算（タイプミスを修正）
   Quadruple operator-(const Quadruple& rhs) const {
      return *this + Quadruple(-rhs.hi, -rhs.lo);
   }
...
void quadruple_adamard_product_add(const std::array<std::array<std::complex<Quadruple>, M>, N>& a,
                                   const std::array<std::array<std::complex<Quadruple>, M>, N>& b,
                                   std::array<std::array<std::complex<Quadruple>, M>, N>& result) {
...
// \label{Fourier2D}
template <typename INPUT, typename TREATED, size_t Nk, size_t Mk, size_t Ns, size_t Ms>
struct Fourier2DConvolution {
   ...
   Fourier2DConvolution(const std::array<std::array<TYPE, COLS>, ROWS>& sample2DIN, const TREATED coeff, const bool reverse0 = false, const bool reverse1 = false) {
      this->initialize_polar_1_c_k();
      this->MatrixDFT = dft_fill(sample2DIN, reverse0, reverse1) * coeff;
   }

   template <typename TYPE, size_t ROWS, size_t COLS>
   Fourier2DConvolution(const std::array<std::array<TYPE, COLS>, ROWS>& sample2DIN, const bool reverse0 = false, const bool reverse1 = false) {
      this->initialize_polar_1_c_k();
   ...
   template <typename TYPE, size_t ROWS, size_t COLS>
   void add(const std::array<std::array<TYPE, COLS>, ROWS>& sample2DIN) {
   ...
   template <typename TYPE, size_t ROWS, size_t COLS>
   void add(const std::array<std::array<TYPE, COLS>, ROWS>& sample2DIN, const TREATED coeff) {
      static_assert(ROWS <= rows && COLS <= cols, "Input size exceeds the defined size of Fourier2DConvolution.");
   ...
   void adamard_product_add(const Fourier2DConvolution<ANYTYPE, TREATED, Nk, Mk, Ns, Ms>& inputDFT2,
                            const Fourier2DConvolution<ANYTYPE, TREATED, Nk, Mk, Ns, Ms>& inputDFT1) {
      this->MatrixDFT += inputDFT2.MatrixDFT * inputDFT1.MatrixDFT;
   };
   ...
   void adamard_product_add(const Fourier2DConvolution<ANYTYPE, TREATED, Nk, Mk, Ns, Ms>& inputDFT2,
                            const Fourier2DConvolution<ANYTYPE, TREATED, Nk, Mk, Ns, Ms>& inputDFT1,
                            const TREATED coeff) {
      this->MatrixDFT += inputDFT2.MatrixDFT * inputDFT1.MatrixDFT * coeff;
   };
   ...
   std::array<std::complex<TREATED>, N> InverseDFT(const std::array<TYPE, N>& sample) {
   ...
   std::array<std::array<std::complex<TREATED>, cols>, rows> InverseDFT(const std::array<std::array<TYPE, cols>, rows>& sample2D) {
   ...
   template <typename TYPE, size_t COLS>
   std::array<std::complex<TREATED>, cols> dft1d_cols(const std::array<TYPE, COLS>& sample) {
   ...
   std::array<std::complex<TREATED>, rows> dft1d_rows(const std::array<TYPE, ROWS>& sample) {
   ...
   template <typename TYPE, size_t ROWS, size_t COLS>
   std::array<std::array<std::complex<TREATED>, cols>, rows> dft_fill(const std::array<std::array<TYPE, COLS>, ROWS>& sample2DIN, bool reverse0 = false, bool reverse1 = false) {
      static_assert(ROWS <= rows && COLS <= cols, "Input size exceeds the defined size of Fourier2DConvolution.");
...
template <typename T>
std::vector<std::vector<std::complex<double>>> InverseDFT(const Fourier2D<T>& convolver2Dkernel) {
   return InverseDFT(convolver2Dkernel.MatrixDFT);
}
...
template <typename T>
std::vector<std::vector<std::complex<double>>> Convolution(const Fourier2D<T>& convolver2Dkernel, const Fourier2D<T>& convolver2Dshift) {
   return InverseDFT(convolver2Dkernel.MatrixDFT * convolver2Dshift.MatrixDFT);
}

template <typename T>
std::vector<std::vector<double>> ReConvolution(const Fourier2D<T>& convolver2Dkernel, const Fourier2D<T>& convolver2Dshift) {
...
struct Convolver2D {
   ...
   void convolve() {
      convolution = InverseDFT(kernelDFT * shiftDFT);
   };
