...
//    // #ifdef TETLIBRARY
//    throw std::runtime_error("TetGen: Terminated with error code " + std::to_string(x));
//    // #else
//    //    switch (x) {
...
inline void terminatetetgen(tetgenmesh* /*m*/, int x) {
   char buffer[50];
   snprintf(buffer, sizeof(buffer), "TetGen: Terminated with error code %d", x);
