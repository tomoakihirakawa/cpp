...
// std::array に対する特殊化
template <typename T_val, std::size_t N>
struct vector_traits<std::array<T_val, N>> {
   ...
   // サイズを取得する静的関数
   static constexpr std::size_t size(const std::array<T_val, N> &) {
      return N;
   }
};
