...
std::vector<double> fOkada(const std::vector<double>& U,
			   ...
			   const double mu,
			   const double lam){
  double cosdel = cos(del);
  double sindel = sin(del);  
  ...
  double I5 = (abs(xi_eta[0]) < 10E-10) ? 0 : mu/(lam + mu)*2/cosdel*atan((xi_eta[1]*(X + q*cosdel) + X*(R + X) * sindel)/(xi_eta[0]*(R + X)*cosdel));
  double I4 = mu/(lam + mu)/cosdel*(log(R + tild) - sindel*log(R + xi_eta[1]));
  double I3 = mu/(lam + mu)*(tily/(cosdel*R + tild) - log(R + xi_eta[1])) +  tan(del)*I4;
  double I2 = mu/(lam + mu)*(-log(R + xi_eta[1])) - I3;
  double I1 = mu/(lam + mu)*(-xi_eta[0]/(cosdel*(R + tild))) - tan(del)*I5;
  double tmp = (abs(q)  < 10E-10) ? 0. : atan(xi_eta[0]*xi_eta[1]/(q*R));
  return {
...
std::vector<double> IntegrateStrikeDipOkada(const std::vector<double>& U,
  ...
  std::vector<std::vector<double>>A = {{cos(SKRad[0]), -sin(SKRad[0])}, {sin(SKRad[0]), cos(SKRad[0])}};
  std::vector<double> xy = Dot(A,xyIN-xyIN_orign);
  double x = xy[1];
  double y = -xy[0];
  double p = y*cos(SKRad[1]) + d*sin(SKRad[1]);
  std::vector<double> ret = fOkada(U, y, {x, p + LW[1]}, SKRad[1], d, mu, lam)
    - fOkada(U, y, {x, p}, SKRad[1], d, mu, lam)
    - fOkada(U, y, {x - LW[0], p + LW[1]}, SKRad[1], d, mu, lam)
    + fOkada(U, y, {x - LW[0], p}, SKRad[1], d, mu, lam);
...
//     double x = r * cos(160.*M_PI/180.) + 63.8479;
//     double y = r * sin(160.*M_PI/180.) - 31.7988;
