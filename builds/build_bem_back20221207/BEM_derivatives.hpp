   ...
   auto closestXFacing = [](const Tddd &p_next_X, const double radius, const std::vector<T3Tddd> &vertices, const Tddd &n) {
      ...
      for (const auto &vertex : vertices) {
         if (isFacing(TriangleNormal(vertex), n, M_PI / 180 * 20)) {
            ...
            if (radius > Norm(X - p_next_X) && Norm(r) >= Norm(X - p_next_X))
               r = X - p_next_X;
         }
      }
      return r;
...
Tddd vectorToNearestAjacentSurface(const networkPoint *p) {
   /*
   UartificialClingは，完璧にOmega(t+\delta t)に張り付くようにしなければ，
   面からはなれることで計算の破綻を招く可能性がある．
   */
   ...
   if (p->CORNER) {
      ...
      for (const auto &l : p->getLines())
         if (l->CORNER) {
            X = Nearest(pX, T2Tddd{p->getXBuffer(), (*l)(p)->getXBuffer()});
            if (Norm(ret - pX) >= Norm(X - pX))
               ret = X;
         }
   } else {
      for (const auto &f : p->getFaces()) {
         if ((p->Dirichlet && f->Dirichlet) || (p->Neumann && f->Neumann)) {
            auto [p0, p1, p2] = f->getPoints(p);
            X = Nearest(pX, T3Tddd{p0->getXBuffer(), p1->getXBuffer(), p2->getXBuffer()});
            if (Norm(ret - pX) >= Norm(X - pX))
               ret = X;
         }
      }
...
#pragma omp single nowait
         ...
         if (p->CORNER || (p->Dirichlet && adjust_dirichlet)) {
            if (isFinite(p->U_BUFFER_BUFFER)) {
               ...
               p->U_BUFFER_BUFFER = (p->clungSurface - (p->getXBuffer() + p->U_BUFFER));
               //! 角のノイマン面から離れるのを防ぐ
               if (p->CORNER)
                  for (const auto &f : p->getFacesNeumann())
            ...
            if (p->CORNER || p->Neumann) {
               {
                  ...
                  p->U_BUFFER_BUFFER = vectorsToSurfaceFromBufferX(p, nextBodyVertices(Fs));
                  //! 角のディリクレ面へのめり込みを防止
                  if (p->CORNER)
