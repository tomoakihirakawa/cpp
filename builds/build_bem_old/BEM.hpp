...
VV_VarForOutput dataForOutput(const Network *water, const double dt) {
   try {
      ...
      uomap_P_Tddd P_whereToGo_to_check = p_tdd0;
...
#pragma omp single nowait
         {
            ...
            P_whereToGo_to_check[p] = p->whereToGo_to_check = findOptimalPositionByCriteriaOn(p, criteria, get_base_X) - p->X;
      ...
      try {
         VV_VarForOutput data = {
             ...
             {"position", P_position},
             {"whereToGo_to_check", P_whereToGo_to_check},
             {"φ", P_phi},
             {"φn", P_phin},
             {"φt", P_phi_t},
             {"φnt", P_phin_t},
      ...
      for (const auto &P : p->getNeighbors())
         for (const auto &q : P->getNeighbors())
            if (p != q) {
               ...
               dt = c * Distance(p, q) / Norm(p->U_BEM - q->U_BEM);
