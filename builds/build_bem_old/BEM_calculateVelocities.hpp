...
std::vector<T3Tddd> nextBodyVertices(const auto &Fs) {
...
// Tddd findOptimalPositionByCriteria_RK_with_Ubuff(const networkPoint *p, const std::function<double(const Tddd &)> &criteria) {
//    Tddd X0_cadidate, where_the_min_score = RK_with_Ubuff(p);  //% 繰り返し計算して得られた今のところ最適な位置，RK_with_Ubuffでないといけない．
//    double min_score = criteria(where_the_min_score), s;
//    for (const auto &f : p->getFaces()) {
//       auto [p0, p1, p2] = f->getPoints(p);
//       Tddd X0_true = RK_without_Ubuff(p0);  //! must be RK_without_Ubuff
//       Tddd X1_true = RK_without_Ubuff(p1);  //! must be RK_without_Ubuff
//       Tddd X2_true = RK_without_Ubuff(p2);  //! must be RK_without_Ubuff
//       T3Tddd X012_true = {X0_true, X1_true, X2_true};
//       Tddd true_next_normal = TriangleNormal(X012_true);
//       double min_t0, min_t1;
//       // in more detail for min_face
//       std::array<double, 2> t0t1_0 = {0.9, 1.0};
//       std::array<double, 2> t0t1_1 = {0.0, 1.0};
//       for (int i = 0; i < 5; ++i) {
//          for (const auto &[t0, t1, shape] : t0t1ModTriShape25(t0t1_0, t0t1_1)) {
//             X0_cadidate = Dot(shape, X012_true);
//             if (Dot(TriangleNormal(X0_cadidate, X1_true, X2_true), true_next_normal) > 0) {
//                s = criteria(X0_cadidate);
//                if (isFinite(s) && min_score > s) {
//                   min_score = s;
//                   where_the_min_score = X0_cadidate;
//                   min_t0 = t0;
//                   min_t1 = t1;
//                }
//             }
//          }
//          double dt = std::pow(0.1, i + 1);
//          t0t1_0 = {std::clamp(min_t0 - dt, 0.0, 1.0), std::clamp(min_t0 + dt, 0.0, 1.0)};
//          t0t1_1 = {std::clamp(min_t1 - dt, 0.0, 1.0), std::clamp(min_t1 + dt, 0.0, 1.0)};
//       }
//    }
...
void calculateVecToSurface(const std::vector<networkPoint *> &points, const int loop, const bool do_shifting = true) {
...
void calculateCurrentUpdateVelocities(const Network &net, const int loop, const bool do_shifting = true) {
   ...
   auto points = ToVector(net.getPoints());
   calculateVecToSurface(points, loop, do_shifting);
   ...
   //    for (const auto &p : points) {
   //       double area = 0;
   //       std::ranges::for_each(p->getFaces(), [&](const auto &f) { area += f->area; });
   //       if (Norm(p->vecToSurface) > std::sqrt(area / M_PI) * 0.5) {
   //          if (i == N - 1)
   //             p->shift_coeff *= 0.;
   //          else
   //             p->shift_coeff *= 0.5;
   ...
   //    }
   //    if (found)
   //       calculateVecToSurface(points, loop);
   //    else
   //       break;
   // }
   ...
   for (const auto &p : net.getPoints()) {
      ...
      p->U_update_BEM = p->RK_X.toReachAtNextTimeQ(p->X + p->vecToSurface);
      // p->U_update_BEM += p->vecToSurface / p->RK_X.getdt();
   }
}
