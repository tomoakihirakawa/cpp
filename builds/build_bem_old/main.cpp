...
int main(int argc, char **argv) {
   ...
   */
   ...
   auto setup_network_output_info = [&NetOutputInfo](Network *net, auto output_name, const std::filesystem::path &output_directory) {
      ...
      auto tmp = output_directory / (output_name + ".pvd");
      NetOutputInfo[net].PVD = new PVDWriter(tmp);
...
#pragma omp single nowait
            ...
            for (auto water : FluidObject) {
               // calculateCurrentUpdateVelocities(*water, 10, 1);
...
#pragma omp single nowait
            {
               ...
               if (net->inputJSON.find("velocity")) {
                  ...
                  for (auto &mooring : net->mooringLines) {
                     ...
                     const double dt = net->RK_COM.getdt();
                     Tddd V = (nextPositionOnBody(net, mooring->lastPoint) - Xcurrent) / dt;
                     mooring->simulate(simulation_time, dt, [&](networkPoint *p) {
