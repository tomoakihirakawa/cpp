...
int main(int arg, char **argv) {
   ...
   std::cout << "data1D.size() = " << buckets.data1D.size() << std::endl;
   std::cout << "timer : " << timer() << std::endl;
   /* -------------------------------------------------------------------------- */
   ...
   for (auto k = 0; k < 10; k++) {
      ...
      for (const auto &f : faces) {
         ...
         */
         ...
         map_points_inside[nullptr] = {first_X, score(first_X)};
         ...
         // 対象となる点の候補を取得
         {
            ...
            auto Xc = Circumcenter(Append(ToX(f), X));
            // 異常に大きい半径のものは，排除する
            if (r / Mean(extLength(f->getLines())) < 5)
         ...
         // if (points_inside.empty() && condition(first_X)) {
         //    double r = 0.01;

         //    // auto Xc = Circumcenter(Append(ToX(f), first_X));

         //    double min_dist = 1E+10;
         //    networkPoint *closest_p = nullptr;
         //    buckets.apply(first_X, r, [&](const auto &p) {
         //       if (Norm(p->X - first_X) < r && !MemberQ(f->getPoints(), p) && condition(p->X)) {
         //          min_dist = Norm(p->X - first_X);
         //          closest_p = p;
         //       }
         //    });
         ...
         //       if (points_inside.empty() && !MemberQ(f->getPoints(), p) && condition(p->X)) {
         //          if (object->isInside_MethodBucket(Incenter(Append(ToX(f), p->X)))) {
         //             gen_tetra(p);
         //             break;
         //          }
         //       }
