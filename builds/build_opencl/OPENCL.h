...
struct OPENCL_kernel
{
  ...
  cl_program sources;
  cl_program program;
  cl_command_queue queue; /// vector is not needed??
  cl_kernel kernel;
  std::vector<cl_buffer> buffers;

  OPENCL_kernel(const std::string &kernel_codeIN, const std::string &func_nameIN) : kernel_code(kernel_codeIN),
                                                                                    func_name(func_nameIN)
  {
    ...
    ///////////////////////////////////////
    context = clContext({device});
    sources.push_back({kernel_code.c_str(), kernel_code.length()});
    program = clProgram(context, sources);
    ...
    ///////////////////////////////////////
    queue = clCommandQueue(context, device);

    // set up kernels and vectors for GPU code
    kernel = clKernel(program, func_name.c_str());
  ...
  {
    queue.enqueueNDRangeKernel(kernel, cl_ndrange(offset), cl_ndrange(work_item), cl_ndrange(num));
  };
  //========================================
  void enqueueNDRangeKernel(const int work_item, const int num)
  {
    queue.enqueueNDRangeKernel(kernel, clNullRange, cl_ndrange(work_item), cl_ndrange(num));
  };
  //========================================
  template <typename T>
