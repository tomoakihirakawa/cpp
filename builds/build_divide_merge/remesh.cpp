...
int main(int arg, char **argv) {
   ...
   V_netLp lines;
   Histogram Histo;
   std::array<std::string, 4> icons = {Red + '|' + colorOff, Magenta + '/' + colorOff, Blue + '-' + colorOff, Green + '\\' + colorOff};
   for (auto count = 0; count <= remesh; ++count) {
      ...
      std::cout << "\r" << icons[(count + 1) % icons.size()] << time() << std::flush;
      ...
      */
      ...
      //   std::cout << "\r1. time:" << time() << std::flush;
      AreaWeightedSmoothingPreserveShape(net.getPoints(), small);
      for (const auto &l : net.Lines)
         l->flipIfBetter(M_PI / 180.);
      //   std::cout << "\r2. time:" << time() << std::flush;
      AreaWeightedSmoothingPreserveShape(net.getPoints(), small);
      //   std::cout << "\r \r" << std::flush;
      //   std::cout << "\r3. time:" << time() << std::flush;
