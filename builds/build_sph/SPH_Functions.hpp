...
void setSML(const auto& target_nets, const std::array<double, 2> C_SML_min_max = { 2.7, 2.7 }) {
    ...
    for (const auto& NET : target_nets)
    {
        const auto& points = ToVector(NET->getPoints());
        if (NET->isFluid) {
...
double dt_CFL(const double dt_IN, const auto& net, const auto& RigidBodyObject) {
    // double dt = dt_IN;
    const auto C_CFL_velocity = 0.15;           // dt = C_CFL_velocity*h/Max(U)
    const auto C_CFL_accel = 0.15;              // dt = C_CFL_accel*sqrt(h/Max(A))
    const auto C_CFL_viscosity = 0.15;          // dt = C_CFL_viscosity*h^2/Max(Viscosity)
    const auto C_CFL_velocity_relative = 0.15;  // dt = C_CFL_velocity*h/Max(U)
    const auto C_CFL_accel_relative = 0.15;     // dt = C_CFL_accel*sqrt(h/Max(A))j
    // 音速
#pragma omp parallel for
...
double rho_next(const networkPoint* p) {
    ...
    return 0.01 * p->intp_density + 0.49 * (_WATER_DENSITY_ + rho);
    // return a * rho + (1. - a) * _WATER_DENSITY_;
...
#pragma omp parallel for
        for (const auto& p : ToVector(points))
        {
                ...
                for (const auto& [closest_p, f2w] : { findClosest(p, vector_points) /* , closest_next(p, RigidBodyObject)*/ }) {
                    if (closest_p != nullptr) {
                        ...
                        if (dist_f2w < particle_spacing) {
                            if (normal_dist_f2w < 0.85 * particle_spacing && Dot(p->U_SPH, n) < 0) {
