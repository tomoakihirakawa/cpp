...
#pragma omp single nowait
   {
      ...
      */
      ...
      auto EISPH_wall_pressure = [&](auto &total_weight, auto &pressure_for_wall) {
         total_weight = 0;
         pressure_for_wall = 0;
         double dP = 0;
            ...
            if (!net->isRigidBody) {
               net->BucketPoints.apply(markerX, ROW->radius_SPH, [&](const auto &B) {
                  ...
                  if (Distance(markerX, B->X) < ROW->radius_SPH) {
                     auto w = B->volume * w_Bspline(Norm(B->X - markerX), ROW->radius_SPH);
      ...
      auto ISPH_wall_pressure = [&](const auto &B /*column id*/, const double &coef = 1.) {
         ...
         if (!B->isAir)
            if (Distance(markerX, B->X) < ROW->radius_SPH) {
               auto w = B->volume * w_Bspline(Norm(B->X - markerX), ROW->radius_SPH);
               ROW->increment(B, w);
               auto dP = Dot(Projection(ROW->X - B->X, Normalize(ROW->normal_SPH)), B->mu_SPH * B->lap_U + B->rho * _GRAVITY3_);
               ROW->PoissonRHS -= dP * w;
            }
      };
...
//
      ...
      */
      ...
      if (ROW->isAir || ROW->isSurface) {
         ...
         // b% EISPH
         EISPH_wall_pressure(total_weight, pressure_for_wall);
