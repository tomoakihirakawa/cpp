...
std::vector<networkPoint*> wall_p;
...
#pragma omp parallel for
        ...
        do {
            ...
            /* -------------------------------------------------------------------------- */
            //% 粘性項の計算
            calcLaplacianU(Join(points, wall_p), Append(net_RigidBody, net));
            ...
            /* -------------------------------------------------------------------------- */
            //% Poisson方程式の離散化
            setPoissonEquation(Join(points, wall_p), Append(net_RigidBody, net), particle_spacing);
            Print(Green, "setPoissonEquation", Blue, "\nElapsed time: ", Red, watch(), colorReset, " s");
            DebugPrint(Red, __FILE__, " ", __PRETTY_FUNCTION__, " ", __LINE__);
            /* -------------------------------------------------------------------------- */
            //% 圧力 p^n+1の計算
#if defined(USE_ISPH)
            solvePoisson(Join(points, wall_p));
            ...
            gradP(points, Append(net_RigidBody, net));
            Print(Green, "calculate grad(P) and increment DU/Dt", Blue, "\nElapsed time: ", Red, watch(), colorReset, " s");
            DebugPrint(Red, __FILE__, " ", __PRETTY_FUNCTION__, " ", __LINE__);
            //! NEW -------------------------------------------------------------------------- */
            // calculate_nabla_otimes_U_next(Join(points, wall_p), Append(net_RigidBody, net));
            /* -------------------------------------------------------------------------- */
            //% 粒子の時間発展
