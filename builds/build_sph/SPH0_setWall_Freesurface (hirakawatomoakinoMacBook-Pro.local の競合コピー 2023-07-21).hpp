         ...
         obj->BucketPoints.apply(p->X, captureRange, [&](const auto &q) {
            if (Distance(p, q) < captureRange) {
               ...
               q->isNeumannSurface = net->BucketPoints.any_of(q->X, q->radius_SPH, [&](const auto &b) {
                  return Distance(q, b) < r_short && q != b && (VectorAngle(q->interpolated_normal_SPH_original, b->X - q->X) < M_PI / 5);
               });
...
#pragma omp single nowait
   {
      ...
      */
      ...
      auto surface_condition1 = [&](const auto &q) {
         return Distance(p, q) < radius && p != q && (VectorAngle(p->interpolated_normal_SPH_original, -q->normal_SPH) < M_PI / 180. * 45);
      };
