...
void setCorrectionMatrix(const auto &all_nets) {
   ...
   */
   ...
   auto setCorrectionMatrix_gradient = [&all_nets](networkPoint *A) {
      ...
      if (!isFinite(A->var_Eigenvalues_of_M1) || A->var_Eigenvalues_of_M1 > 1E+2 || !isFinite(A->inv_grad_corr_M, 1e+13)) {
         IdentityMatrix(A->inv_grad_corr_M);
         A->is_grad_corr_M_singular = true;
      }

      if (!isFinite(A->var_Eigenvalues_of_M1_next) || A->var_Eigenvalues_of_M1_next > 1E+2 || !isFinite(A->inv_grad_corr_M_next, 1e+13)) {
         IdentityMatrix(A->inv_grad_corr_M_next);
         A->is_grad_corr_M_next_singular = true;
      }
   };
...
#pragma omp single nowait
         if (A->isCaptured) {
            ...
            */
            ...
            const double surface_check_r_for_fluid_ = A->particle_spacing * 2.7;            // 流体粒子に対してはr=2.5*ps，\theta=30度
            const double surface_check_r_for_wall_ = A->particle_spacing * std::sqrt(2.7);  // 壁粒子に対してはr=2*ps，\theta=30度．理由壁は長く伸びていることがあるため，長距離の粒子を認識しないようにするため．
            ...
            double surface_check_r_for_fluid = surface_check_r_for_fluid_;  // * (1. - A->var_Eigenvalues_of_M);
            double surface_check_r_for_wall = surface_check_r_for_wall_;    // * (1. - A->var_Eigenvalues_of_M);
            // }
            ...
            // {
            //! 分布が均等でないものは水面になりやすくする
            surface_check_r_for_fluid = surface_check_r_for_fluid_;  // * (1. - A->var_Eigenvalues_of_M_next);
            surface_check_r_for_wall = surface_check_r_for_wall_;    // * (1. - A->var_Eigenvalues_of_M_next);
            // }
