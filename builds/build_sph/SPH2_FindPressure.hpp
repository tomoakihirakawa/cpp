...
void setPressure(const auto& points) {
...
void setPoissonEquation(const auto& pointsIN,
...
void solvePoisson(const auto& all_particle) {
    DebugPrint(Blue, "solvePoisson", __FILE__, " ", __PRETTY_FUNCTION__, " ", __LINE__);
    std::vector<networkPoint*> points = all_particle;
