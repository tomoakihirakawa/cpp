...
#pragma omp single nowait
         {
            ...
            applyOverPoints([&](const auto &Q) {
               if (canInteract(A, Q) && A != Q) {
                  A->tensorproduct_grad_Uij += Q->volume * TensorProduct(grad_w_Bspline(A, Q), Q->U_SPH - A->U_SPH);
                  A->tensorproduct_grad_Uij_next += V_next(Q) * TensorProduct(grad_w_Bspline_next(A, Q), U_next(Q) - U_next(A));
               }
            });
...
#pragma omp single nowait
         {
            ...
            if (A->getNetwork()->isRigidBody) {
               ...
               A->b_vector = A->rho * (A->U_SPH / dt + A->DUDt_SPH - _GRAVITY3_);
            } else {
               A->DUDt_SPH = A->mu_SPH / A->rho * A->lap_U + _GRAVITY3_;  //! 後で修正されるDUDt
               A->DUDt_SPH += A->U_XSPH / dt;
               //
...
#pragma omp single nowait
            if (A->isFluid) {
               ...
               // 修正１
               //  auto V = A->interp_normal_original_next;
               //  auto [Vn, Vs] = DecomposeVector(V, A->interp_normal_next);
                  ...
                  if (A->isFluid && B->isFluid) {
                     ...
                     if (Norm(vec_A2B) < A->particle_spacing) {
                        ...
                        if (Dot(relative_velocity, vec_A2B) < 0)
                           A->DUDt_modify_SPH -= 0.1 * Projection(relative_velocity, vec_A2B) / dt;
                     }
                  }
               };
