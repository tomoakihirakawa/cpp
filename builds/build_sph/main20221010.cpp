...
#elif defined(EISPH)
            p->RK_pressure.initialize(dt, real_time, p->pressure_SPH, RK_order);
#endif
         }
         /* ------------------------------------------------------ */
         for (const auto &n : Join(RigidBodyObject, {net}))
...
#pragma omp single nowait
            ...
            for (const auto &p : water_points)
               p->pressure_SPH = p->RK_pressure.getXinit();
               // b! ------------------------------------------------------ */
               // b!  　　　　　           仮の圧力Pの計算                          　　　　*/
               // b! ------------------------------------------------------ */
#pragma omp parallel
...
#elif defined(EISPH)
               //圧力
               p->RK_pressure.push(p->DPDt_SPH);
               p->setDensity(p->RK_pressure.getX());
