from math import pi
import json
import math

# edge_length = 2.*0.05

H = 300/1000
particle_spacing = 0.008
data = {
    # -------------------------------------------------------- #
    "initial_surface_height": 0.8,
    # ---------------- 時間間隔dtに関する設定値 ----------------- #
    # 最初のstep==0の場合はdt=1E-10とする
    "C_CFL_velocity": 0.04,  # dt = C_CFL_velocity*h/Max(U)
    "C_CFL_accel": 0.25,     # dt = C_CFL_accel*sqrt(h/Max(A))
    "max_dt": 0.01,  # 1/1500(speed of sound) = 0.0006666666666666666
    # ------------------ 粒子配置に関する設定値 ------------------ #
    "particle_spacing": particle_spacing,
    "xbounds": [205/1000 + particle_spacing/2, 805/1000 - particle_spacing/2],
    "ybounds": [-75/1000 + particle_spacing/2, 75/1000 - particle_spacing/2],
    "zbounds": [500/1000 + particle_spacing/2, 500/1000+H + particle_spacing/2],
    # -------------------------------------------------------- #
    # バケットのバウンディングボックスの外に達した流体粒子は削除する．
    "buckets_xbounds": [-0.9, 0.9],
    "buckets_ybounds": [-.14, .14],
    "buckets_zbounds": [0.4, 1.],
    #@ ---------------------- 平滑化半径に関するの設定値（計算精度に関わる） --------------------- #
    "C_SML": .8,  # 一般的な平滑化距離．5次のスプラインの場合3h離れた粒子は影しない
    "kNS_SML": 7,  # k-nearest search. dxを決めるための近傍粒子数
    # -------------------------------------------------------- #
    "mu": 0.001005,
    # -------------------------------------------------------- #
    "C_Tait": 1466/60.,  # テイトの式
    # ---------------------- 人口粘性係数 ---------------------- #
    "C_artificial_viscousity_alpha": 0.03,
    "C_artificial_viscousity_beta": 0.,
    # ------------------------ 準備時間 ------------------------ #
    "preparation_max_dt": 0.001,
    "preparation_time": 1.,
    "preparation_time_step": 5,
    "preparation_C_artificial_viscousity_alpha": 0.2,
    "preparation_C_artificial_viscousity_beta": 0.2,
    # -------------------------------------------------------- #
}

f = open("./settingSPH.json", 'w')
json.dump(data, f, ensure_ascii=True, indent=4)
f.close()

xlen = data["xbounds"][1]-data["xbounds"][0]
ylen = data["ybounds"][1]-data["ybounds"][0]
zlen = data["zbounds"][1]-data["zbounds"][0]
dx = data["particle_spacing"]
print("粒子点数", (round(xlen/dx+1)*round(ylen/dx+1)*round(zlen/dx+1)))
print("rho*g*h = ", 1000.*9.81*H)

#! -------------------------------------------------------- #
#! -------------------------------------------------------- #
#! -------------------------------------------------------- #

data = {
    "name": "tank",
    "objfile": "../../obj/watanabe2021/tank_watanabe16.obj",
    "rotate": [2*math.pi, 1, 0, 0],
    "scale": [1/1000, 0, 0, 0],  # モデルがmm単位なのでメートルに変換,
    "depth_list": [-particle_spacing/2.,
                   -particle_spacing/2.*3.,
                   -particle_spacing/2.*5.,
                   -particle_spacing/2.*7.,
                   -particle_spacing/2.*9.]

}

f = open("./tank.json", 'w')
json.dump(data, f, ensure_ascii=True, indent=4)
f.close()


#! -------------------------------------------------------- #
#! -------------------------------------------------------- #
#! -------------------------------------------------------- #

data = {
    "name": "tank",
    "objfile": "../../obj/watanabe2021/tank_watanabe_init16.obj",
    # "rotate": [math.pi/2., 1, 0, 0],
    "scale": [1/1000, 0, 0, 0],  # モデルがmm単位なのでメートルに変換,
    "depth_list": [-particle_spacing/2.,
                   -particle_spacing/2.*3.,
                   -particle_spacing/2.*5.,
                   -particle_spacing/2.*7.,
                   -particle_spacing/2.*9.]

}

f = open("./tank_init.json", 'w')
json.dump(data, f, ensure_ascii=True, indent=4)
f.close()
