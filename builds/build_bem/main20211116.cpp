...
//
// double phin(networkPoint* p, Tddd normal)
// {
// 	normal = Normalize(normal);
// 	return  Dot(p->getNetwork()->forced_velocity,
// 		T6d {
// 		std::get<0>(normal),
// 			std::get<1>(normal),
// 			std::get<2>(normal),
...
// 	});
// };
// double phin(networkPoint* p)
// {
// 	Tddd normal = p->normalContanctSurface(1., 2.);
// 	return  Dot(p->getNetwork()->forced_velocity,
// 		T6d {
// 		std::get<0>(normal),
// 			std::get<1>(normal),
...
// double phin_contact(networkPoint* p)
// {
// 	/* -------------------- contactfaces -------------------- */
// 	std::unordered_set<networkFace*> contactfaces;
// 	for (auto& f : p->getContactFaces()) {
...
// 		auto U_normal_of_face = Dot(f->getNetwork()->velocity, ToT6d(n_face)) * n_face;
// 		auto divide = Dot(n_face, n_point);
// 		// if (!std::abs(divide) < 1E-10/*ほぼ直角*/) {
// 		// 	ret += Norm(U_normal_of_face) / divide;
// 		// 	count++;
// 		// }
// 		ret += Dot(U_normal_of_face, n_point);
// 		count++;
...
/* ------------------------------------------------------ */
/////////////////////////////////////////////////////////////////////////////
// void monitor(GNUPLOT& plot, const BEM::CompGrid& cpg, int step)
// {
// 	V_netFp Faces = cpg.well_Faces;
// 	V_netPp InnerP = cpg.InnerP;
// 	V_netPp CornerP = cpg.CornerP;
// 	V_netPp OuterP = cpg.OuterP;
// 	//----------------------- monitor & plot ------------------------
// 	std::cout << "  Faces :" << Faces.size() << std::endl;
// 	std::cout << " InnerP :" << InnerP.size() << std::endl;
// 	std::cout << "CornerP :" << CornerP.size() << std::endl;
// 	std::cout << " OuterP :" << OuterP.size() << std::endl;
// 	std::string title = "'step:" + std::to_string(step) +
// 		",  Faces:" + std::to_string(Faces.size()) +
// 		",  InnerP:" + std::to_string(InnerP.size()) +
// 		",  CornerP:" + std::to_string(CornerP.size()) +
// 		",  OuterP:" + std::to_string(OuterP.size()) + "'";
// 	plot.Set({ {"key", ""}, {"title", title} });
// 	plot.SaveData(obj3D::extractX(InnerP), { {"ps", "1.2"}, {"pt", "7"}, {"lc", "'blue'"}, {"title", "InnerP"} });
// 	plot.SaveData(obj3D::extractX(CornerP), { {"ps", "1.2"}, {"pt", "7"}, {"lc", "'pink'"}, {"title", "CornerP"} });
// 	plot.SaveData(obj3D::extractX(OuterP), { {"ps", "1.2"}, {"pt", "8"}, {"lc", "'red'"}, {"title", "OuterP"} });
// 	plot.plot3d();
// 	//          std::cin.ignore();
// 	plot.Clear();
// };
...
////////////////////////////
bool coarsen(netLp l, double len)
{
	try
	{
		if (l->length() < len)
		{
			for (const auto &p : l->getPoints())
				if (!isFlat(p, 5.) || !isInConvexPolygon(p))
					return false;
			...
			if (q)
			{
				for (auto &p : q->getNeighbors())
					for (auto &line : getLinesAround(p))
					{
						if (isFlat(line))
						{
							line->flipIfIllegal();
...
/* ------------------------------------------------------ */
// void remesh(Network* obj0, const double lim_len)
// {
// 	// auto obj0 = all_obj[i];
// 	display(obj0);
// 	bool found;
...
// 			mk_vtu(output_directory + "/remesh" + std::to_string(count++) + ".vtu", obj0->getFaces());
// 		}
// 	}
// 	catch (error_message& e)
// 	{
...
#pragma omp single nowait
#endif
		{
			...
			/* ------------------------------------------------------ */
			// if ((p->CORNER || p->Neumann) && !p->getContactFaces().empty())
			// 	for (const auto& f : p->getContactFaces())
			// 	{
			// 		auto n = f->getNormalTuple();
			// 		tension -= Dot(tension, n) * n;
			// 	}
			// if (p->Neumann)
			// 	for (const auto& f : p->getFaces())
			// 	{
			...
			bool isFlat = true;
			// for (const auto& f : p->getFaces())
			// 	if (50. * M_PI / 180. < std::abs(MyVectorAngle(f->getNormalTuple(), tangent) - M_PI / 2.))
			// 		isFlat = false;

			// if (isFlat)
			// p->U_update_BEM = p->U_BEM + (Norm(tangent) > Norm(p->U_BEM) ? Norm(p->U_BEM) : 1.) * tangent; //p->U_normal_BEM + tangent; //要修正．EMTをプログラム
			//! tensionが強いと角でとがるようなエラーがでる．
			p->U_update_BEM = p->U_BEM + 0.2 * tension; // p->U_normal_BEM + tangent; //要修正．EMTをプログラム

			/* ------------------------------------------------------ */
			// やっぱり過剰決定がないと，うまくいかない？？？
			if ((p->Neumann || p->CORNER) && !p->getContactFaces().empty())
			{
				for (const auto &f : p->getContactFaces())
				{
				...
				// 		auto angle = MyVectorAngle(v, n) / M_PI * 180.;
				// 		if (std::abs(angle) < 10. || std::abs(angle - 180.) < 10.)
				// 			ignore = true;
				// 	}
				// 	if (!ignore) {
				// 		p->U_BEM -= Dot(p->U_BEM, n) * n;
...
public:
	double t0, t1;
	netFp f;
	originFace(const netFp fIN, double t0IN, double t1IN) : f(fIN), t0(t0IN), t1(t1IN){};
};
