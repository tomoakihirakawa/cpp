...
void calculateVectorToClingNeumann(const Network &net, double dt)
{
	/*
	@ ノイマン面に貼り付けるための必要な調整
	*/
...
#pragma omp single nowait
#endif
		{
			...
			if (p->CORNER)
			{
				...
				p->U_mod_BEM = p->U_BEM - Dot(p->U_BEM, tang) * tang;
			}
			else if (p->Neumann)
			{
				// p->U_mod_BEM = fitToNeumannVelocity(p->U_normal_BEM, p);
				p->U_mod_BEM = uNeumann(p);
			}

			for (const auto &q : Points)
				q->X_BUFFER = q->getXtuple() + q->U_mod_BEM * dt;
		}
		/* ------------------------------------------------------ */
#ifdef derivatives_debug
...
#pragma omp single nowait
		...
		*/
		for (const auto &p : Points)
			p->U_update_BEM = p->U_mod_BEM;
		/* ------------------------------------------------------ */
		if (adjust_mesh)
		{
			calculateVectorToSurfaceInBuffer(net, dt);
			for (const auto &p : Points)
				p->U_update_BEM += p->U_BUFFER / dt; // p->U_normal_BEM + tangent; //要修正．EMTをプログラム
		}
		else
		{
			for (const auto &p : Points)
		...
		for (const auto &p : Points)
		{
			p->U_cling_to_Neumann = p->U_cling_to_Neumann_ = p->U_BUFFER / dt; // p->U_normal_BEM + tangent; //要修正．EMTをプログラム
			p->U_update_BEM += p->U_BUFFER / dt;							   // p->U_normal_BEM + tangent; //要修正．EMTをプログラム
		}
		/* ------------------------------------------------------ */
		for (const auto &p : Points)
		{
			// p->U_update_BEM = p->U_mod_BEM + p->U_BUFFER / dt; // p->U_normal_BEM + tangent; //要修正．EMTをプログラム
...
int main()
{
	try
	{
		...
		for (time_step = 0; time_step < 2000; time_step++)
		{
			...
			// double min_dt = 1E+10;
			// double dt = dt_CFL(water, max_dt, {0.2, 5});
			double dt = max_dt;
			...
			do
			{
				...
				// b* ------------------------------------------------------ */
				std::cout << Green << "微分∇ΦやDUDtを計算" << reset << std::endl;
				derivatives ders(*water, dt, time_step % 5 == 0);
				std::cout << Blue << "Elapsed time: " << Red << watch() << reset << " s\n";
				// b* ------------------------------------------------------ */
