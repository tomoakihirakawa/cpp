...
struct SimulationSettings {
  ...
  SimulationSettings(std::filesystem::path input_directory) : input_directory(input_directory), settingJSON(input_directory / setting_filename), output_directory(settingJSON.at("output_directory")[0]), end_time_step(stoi(settingJSON.at("end_time_step"))[0]), end_time(stod(settingJSON.at("end_time"))[0]), max_dt(stod(settingJSON.at("max_dt"))[0]), ALEPERIOD(stoi(settingJSON.at("ALEPERIOD"))[0]) {
    {
      ...
      for (auto input_file_name : settingJSON["input_files"]) {
        ...
        if (!injson.find("ignore") || !stob(injson["ignore"])[0]) {
          ...
          setOutputInfo(injson.at("name")[0], output_directory);            //* for boundary surface */
          setOutputInfo(injson.at("name")[0] + "_tetra", output_directory); //* for volume mesh */
          ...
          for (auto &[key, value] : injson()) {
            if (key.contains("mooring")) {
              ...
              setOutputInfo(name, output_directory);
  ...
  void setTypes(Network *net) {
    ...
    if (type.contains("RigidBody")) {
      ...
      net->inputJSON.find("wave_theory", [&](auto STR_VEC) {
        ...
        net->absorb_gamma = [net](const networkPoint *p) { return std::clamp(p->signed_distance / (3. * net->water_wave_theory.L), 0., 1.); };
      });
      net->inputJSON.find("random_wave_theory", [&](auto STR_VEC) {
        ...
        net->absorb_gamma = [net](const networkPoint *p) { return std::clamp(p->signed_distance / (3. * net->random_water_wave_theory.L13), 0., 1.); };
      });
