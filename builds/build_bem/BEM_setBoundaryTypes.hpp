...
std::size_t setNodeFaceIndices(const Network *objects) { return setNodeFaceIndices(std::vector<Network *>{const_cast<Network *>(objects)}); };
...
void setIsMultipleNode(const auto &p) {
  if (p->CORNER)
    ...
    p->isMultipleNode = std::ranges::any_of(p->getSurfaces(), [&](const auto &f) { return !isFlat(n, f->normal, 20 * M_PI / 180.); });
  }
};
...
void setPhiPhinOnFace(Network *water) {
  const auto surface_vec = water->getSurfaces();
...
#pragma omp parallel for
  for (const auto &p : ToVector(water->getPoints())) {
    ...
    auto run = [p, &alive_faces](networkFace *const &f) {
...
template <typename T1, typename T2, typename T3> void storePhiPhinCommon(const std::vector<Network *> &WATERS, const V_d &ans, T1 phiphinProperty, T2 phiOnFaceProperty, T3 phinOnFaceProperty) {
...
void storePhiPhin(const std::vector<Network *> &WATERS, const V_d &ans) { storePhiPhinCommon(WATERS, ans, &networkPoint::phiphin, &networkPoint::phiOnFace, &networkPoint::phinOnFace); }

void storePhiPhin_t(const std::vector<Network *> &WATERS, const V_d &ans) { storePhiPhinCommon(WATERS, ans, &networkPoint::phiphin_t, &networkPoint::phitOnFace, &networkPoint::phintOnFace); }
