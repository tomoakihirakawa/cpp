...
std::array<double, 3> weight(double t0, double t1) {
  ...
  constexpr std::array<std::array<double, 2>, 3> vertex = {{{std::cos(M_PI / 2), std::sin(M_PI / 2)}, {std::cos(7 * M_PI / 6), std::sin(7 * M_PI / 6)}, {std::cos(11 * M_PI / 6), std::sin(11 * M_PI / 6)}}};
  auto s = shape(t0, t1);
...
#pragma omp parallel for
  ...
  */
  ...
  void initializeBucket() {
    ...
    B_poles.setGrowCondition([](auto bucket) { return bucket->data1D.empty() ? false : (bucket->data1D.size() > 500 && bucket->level < bucket->max_level); });
    std::cout << "Buckets set up complete." << std::endl;
  }

  void createSourcesOnSurfaces() {
    ...
    for (auto &F : obj->getSurfaces()) {
      ...
      if (F->isPseudoQuadraticElement) {
        ...
        */
        ...
        // b% [1] ソース点の配置に関する積分
        for (const auto &[t0t1, quadrature_weight, Nc_N0_N1_N2, X, cross, J_det] : F->map_Point_PseudoQuadraticIntegrationInfo_vector[0].at(p0)) {
          auto get_X = [X]() -> std::array<double, 3> { return X; };
          auto get_normal = [cross]() -> std::array<double, 3> { return Normalize(cross); };
          ...
          auto get_weighted_source_densities = [W = (J_det * quadrature_weight), Nc_N0_N1_N2, pair_pointer_phiphin]() -> std::array<double, 2> {
            ...
            for (int i = 0; i < 4; ++i)
              for (int j = 0; j < 6; ++j) {
                phiWN += W * Nc_N0_N1_N2[i][j] * (*pair_pointer_phiphin[i][j][0]);
                phinWN += W * Nc_N0_N1_N2[i][j] * (*pair_pointer_phiphin[i][j][1]);
              }
          ...
          auto use_this_source_when_set_direct_integration = [F, points = F->Points](const target4FMM *origin) -> std::vector<std::tuple<std::array<double *, 2>, std::array<double, 2>>> {
          ...
          //^ -------------------------------------------------------------------------- */
          auto pole = std::make_shared<source4FMM<target4FMM>>(get_X, get_normal, get_weighted_source_densities, use_this_source_when_set_direct_integration);
          //    B_poles.add(X, pole);
        ...
        for (const auto &[t0, t1, ww] : __array_GW1xGW1__) {
          ...
          auto J_det = Norm(cross);

          auto get_X = [X]() -> std::array<double, 3> { return X; };
          auto get_normal = [cross]() -> std::array<double, 3> { return Normalize(cross); };
          auto get_weighted_source_densities = [W = (J_det * quadrature_weight), shape3 = N012_geometry, pair_pointer_phiphin0, pair_pointer_phiphin1, pair_pointer_phiphin2]() -> std::array<double, 2> { return {W * (shape3[0] * (*pair_pointer_phiphin0[0]) + shape3[1] * (*pair_pointer_phiphin1[0]) + shape3[2] * (*pair_pointer_phiphin2[0])), W * (shape3[0] * (*pair_pointer_phiphin0[1]) + shape3[1] * (*pair_pointer_phiphin1[1]) + shape3[2] * (*pair_pointer_phiphin2[1]))}; };

          //!  ----------------------------- set up direct integration -------------------------------------------- */

          //$ このソース点の直接積分計算をどのように行うかを指定する関数
          auto use_this_source_when_set_direct_integration = [F, obj, points = F->Points, pair_pointer_phiphin0, pair_pointer_phiphin1, pair_pointer_phiphin2](const target4FMM *origin) -> std::vector<std::tuple<std::array<double *, 2>, std::array<double, 2>>> {
            ...
            if (rotate == 1)
              ...
              idx = {1, 2, 0};
            return {{pair_pointer_phiphin0, std::array<double, 2>{WGN[idx[0]], WGnN[idx[0]]}}, {pair_pointer_phiphin1, std::array<double, 2>{WGN[idx[1]], WGnN[idx[1]]}}, {pair_pointer_phiphin2, std::array<double, 2>{WGN[idx[2]], WGnN[idx[2]]}}};
          };
  ...
  std::array<double, 3> solve() {
    ...
    const double time_setPhiPhinOnFace = watch()[0];
    ...
    generateBIEMatrix();
    const double time_setIGIGn = watch()[0];
    std::cout << "BIEの離散化に要した時間: " << Green << time_setIGIGn << colorReset << std::endl;
...
#pragma omp parallel for
    ...
    std::cout << "ツリー構造を生成，またはrebin" << std::endl;
    //   B_poles.generateTree();
    B_poles.rebin();
    std::cout << Magenta << "ツリー構造を生成，またはrebin" << Green << ", Elapsed time : " << tw() << colorReset << std::endl;
    ...
    */
    ...
    {
      std::cout << "copy phi phin" << std::endl;
      copyPhiPhin(obj);
...
#pragma omp parallel for
    ...
    int counter = 1;
    double elapsed_other = 0.;
    std::array<double, 6> elapsed_time{0., 0., 0., 0., 0., 0.};
    std::array<double, 6> tmp_elapsed_time{0., 0., 0., 0., 0., 0.};

    auto AdotV_FMM = [N = 50, this, &counter, &watch, &tmp_elapsed_time, &obj, &GMRES_VECTOR, &points, &elapsed_time, &elapsed_other]() -> V_d {
      tmp_elapsed_time.fill(0.);
      updateFMM(this->B_poles, tmp_elapsed_time);
      elapsed_time += tmp_elapsed_time;
      watch();
#pragma omp parallel for
      for (auto &a : points) {
        for (const auto &[f, i] : a->f2Index) {
          if (a->CORNER && isNeumannID_BEM(a, f)) {
            ...
            auto [GPhin, GnPhi] = (a->integrate() + a->integrateFMM()) / a->diagonal_coefficient;
            GMRES_VECTOR[i] = GPhin - (GnPhi + a->phiOnFace[f]); //! 基本形
          }
        }
      }
      elapsed_other += watch()[0];
      if (counter++ % N == 0) {
        double total = Total(elapsed_time) + elapsed_other;
        std::cout << Blue << "----- Average time for " << N << " iterations -----" << colorReset << std::endl;
        std::cout << "update poles                " << elapsed_time[0] / N << "s " << elapsed_time[0] / total * 100. << " %" << std::endl;
        std::cout << "reset Moments               " << elapsed_time[1] / N << "s " << elapsed_time[1] / total * 100. << " %" << std::endl;
        std::cout << "increment_M_reuse           " << elapsed_time[2] / N << "s " << elapsed_time[2] / total * 100. << " %" << std::endl;
        std::cout << "M2M                         " << elapsed_time[3] / N << "s " << elapsed_time[3] / total * 100. << " %" << std::endl;
        std::cout << "M2L                         " << elapsed_time[4] / N << "s " << elapsed_time[4] / total * 100. << " %" << std::endl;
        std::cout << "L2L                         " << elapsed_time[5] / N << "s " << elapsed_time[5] / total * 100. << " %" << std::endl;
        std::cout << "others (direct integration) " << elapsed_other / N << "s " << elapsed_other / total * 100. << " %" << colorReset << std::endl;
        std::cout << "total                       " << total / N << " s" << std::endl;
        std::cout << "--------------------------------------------------" << std::endl;
        elapsed_time.fill(0.); //! reset elapsed_time
        elapsed_other = 0.;
        counter = 1;
      }
      return GMRES_VECTOR;
...
#pragma omp parallel for
    ...
    std::vector<int> list = {50, 100, 150, 200};
    ...
    for (auto gmres_size : list) {
      std::cout << "Restart count :" << restart_count++ << std::endl;
      TimeWatch local_watch;
      gmres *GMRES = new gmres(return_A_dot_v, b, x0, gmres_size);
      std::cout << "Elapsed time for GMRES: " << local_watch() << " [s] for size = " << gmres_size << std::endl;
      ...
      double E = Norm(ANS() - b);
      //
      std::cout << "GMRES expected error = " << GMRES->err << " True error = " << E << colorReset << std::endl;
      delete GMRES;
      if (E < 1E-8)
        break;
    }
    ...
    const double time_setIGIGn = watch_total()[0];
    const double time_lapack_lu = 0.;

#endif

    std::cout << Green << "Elapsed time for solving BIE: " << Red << watch_from_start() << colorReset << " s\n";

    return {time_setIGIGn, time_lapack_lu, unknownsizse};
  };
...
#pragma omp parallel for
    ...
    */
    ...
    for (const auto &body : rigidbodies)
      if (body->isFloatingBody) {
        ...
        auto func = [&F, &T_GLOBAL, &body](auto key, auto vec_string) {
          std::cout << "Processing key: " << key << " with values: " << vec_string << std::endl;
  ...
  std::vector<double> solveForPhiPhin_t(const std::vector<Network *> &rigidbodies) {
    ...
    BroydenMethod BM(ACCELS_init, ACCELS_init * (1. + 1E-11));
    ...
    for (auto j = 0; j < 50; ++j) {
      ...
      convergence.push_back(Norm(func));
      BM.updateGoodBroyden(func, func_, j == 0 ? 0.1 : alpha);
      // BM.updateBFGS(func, func_, j == 0 ? 0.1 : alpha);
      func_ = func;
      ...
      if (!isFinite(Norm(BM.dX))) {
        for (auto &x : BM.X)
          x = 0;
        for (auto &x : BM.dX)
          x = 1E-10 * RandomReal(0., 1.0);
        insertAcceleration(rigidbodies, BM.X - BM.dX);
        std::cout << "Re-initialize BM.X and BM.dX" << std::endl;
      }

      if (Norm(BM.dX) < 1E-10 && Norm(func) < 1E-10 && count++ > 2)
        break;
#endif
    }
    return convergence;
