...
w_0 w_1
{\left({
\frac{1}{\|{{\bf x}(\mathbfit \xi)}-{\bf a}\|}
\nabla \phi ({\bf{x}}(\mathbfit \xi)) + \phi ({\bf{x}}(\mathbfit \xi))
\frac{{{\bf x}(\mathbfit \xi)}-{\bf a}}{\|{{\bf x}(\mathbfit \xi)}-{\bf a}\|^3}}
\right) \cdot {\bf{n}}({\bf{x}}(\mathbfit \xi))
(1-\xi_0)
}
\end{equation}

ここで${\bf{x}}(\mathbfit \xi)$は，補間パラメタ$\mathbfit \xi$によって表される面上の点である．
