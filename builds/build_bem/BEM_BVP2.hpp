...
#endif
      {
         ...
         for (const auto &integ_f : FMM_BucketsFaces.all_stored_objects) {
            ...
            for_each(ret, [&](const auto &p_igign) {
               int i = -1;
               if ((it = PBF_index.find({std::get<0>(p_igign), integ_f->Dirichlet, integ_f})) != PBF_index.end()) {
                  // これが存在するということは，integ_f
                  i = it->second;
               } else {
                  i = PBF_index[{std::get<0>(p_igign), integ_f->Dirichlet, nullptr}];
               }
               IGIGn_Row[i] += std::get<1>(p_igign);
               /* ------------------------------ */
...

      {
         ...
         for (const auto &[PBF, i] : PBF_index) {
            ...
            if (DorN == Dirichlet)
               ...
               std::get<1>(p->phiphin) += p->phinOnFace[f] * f->area / total;
            } else {
