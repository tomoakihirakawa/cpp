...
std::tuple<netFp, double> NearestContactFaceWithDistance(const networkPoint *const p,
                                                         const networkFace *const f_normal) {
   Tddd r = {1E+100, 1E+100, 1E+100}, X;
   networkFace *ret = nullptr;
   for (const auto &f_target : BFS_Flattened(p->getContactFaces(), 2))
...
Tddd uNeumann(const networkPoint *const p) {
   ...
   for (const auto &f_normal : p->getFaces()) {
      auto [f_hit, distance] = NearestContactFaceWithDistance(p, f_normal);
      if (f_hit) {
         if (f_hit->getNetwork()->isRigidBody) {
            //! value is given for a structure object
            V.push_back(f_hit->getNetwork()->velocityRigidBody(p->X));
            W.push_back(w_Bspline5(distance, p->radius));
         } else if (f_hit->getNetwork()->isSoftBody) {
            //! value is given at nodes of a structure object
            auto [p0, p1, p2] = f_hit->getPoints();
            auto v0 = p0->velocityTranslational();
            auto v1 = p1->velocityTranslational();
            auto v2 = p2->velocityTranslational();
            auto [t0, t1, X] = Nearest_(p->X, ToX(f_hit));
            V.push_back(v0 * t0 + v1 * t1 + v2 * (1 - t0 - t1));
            W.push_back(w_Bspline5(distance, p->radius));
         }
      }
   }
   ...
   /* -------------------------------------------------------------------------- */
   // return optimizeNeumann(p, [&](const auto &f) {
   //    if (f->getNetwork()->isRigidBody) {
   //       return std::tuple<Tddd, double>{
   //           f->getNetwork()->velocityRigidBody(p->X),
   //           w_Bspline5(Norm(Nearest(p->X, ToX(f)) - p->X), p->radius)};
   ...
   //       return std::tuple<Tddd, double>{v0 * t0 + v1 * t1 + v2 * (1 - t0 - t1),
   //                                       w_Bspline5(Norm(p->X - X), p->radius)};
   //    } else
   //       return std::tuple<Tddd, double>{{0., 0., 0.}, 0.};
   // });
};
...
Tddd accelNeumann(const networkPoint *const p) {
   ...
   for (const auto &f_normal : p->getFaces()) {
      ...
      if (f_hit) {
         if (f_hit->getNetwork()->isRigidBody) {
            //! value is given for a structure object
            V.push_back(f_hit->getNetwork()->accelRigidBody(p->X));
            W.push_back(w_Bspline5(distance, p->radius));
         } else if (f_hit->getNetwork()->isSoftBody) {
            //! value is given at nodes of a structure object
            auto [p0, p1, p2] = f_hit->getPoints();
   ...
   //    if (f->getNetwork()->isRigidBody) {
   //       return std::tuple<Tddd, double>{
   //           f->getNetwork()->accelRigidBody(p->X),
   //           w_Bspline5(Norm(Nearest(p->X, ToX(f)) - p->X), p->radius)};
   //    } else if (f->getNetwork()->isSoftBody) {
   //       auto [p0, p1, p2] = f->getPoints();
   //       auto v0 = p0->accelTranslational();
...
Tddd uNeumann(const networkPoint *const p,
              const networkFace *const f_normal) {
   auto f = NearestContactFace(p, f_normal);
   if (f) {
      if (f->getNetwork()->isRigidBody) {
         return f->getNetwork()->velocityRigidBody(p->X);
      } else if (f->getNetwork()->isSoftBody) {
         auto [p0, p1, p2] = f->getPoints();
         auto v0 = p0->velocityTranslational();
         auto v1 = p1->velocityTranslational();
         auto v2 = p2->velocityTranslational();
         auto [t0, t1, X] = Nearest_(p->X, ToX(f));
         return v0 * t0 + v1 * t1 + v2 * (1 - t0 - t1);
      }
   }
   return {0., 0., 0.};
   /* -------------------------------------------------------------------------- */
   // return optimizeNeumann(p, FofP, [&](const auto &f) {
   //    auto [p0, p1, p2] = f->getPoints();
   //    if (f->getNetwork()->isRigidBody) {
   //       return std::tuple<Tddd, double>{
   //           f->getNetwork()->velocityRigidBody(p->X),
...
Tddd accelNeumann(const networkPoint *const p,
   ...
   if (f) {
      if (f->getNetwork()->isRigidBody) {
         return f->getNetwork()->accelRigidBody(p->X);
      } else if (f->getNetwork()->isSoftBody) {
         auto [p0, p1, p2] = f->getPoints();
         auto v0 = p0->accelTranslational();
         auto v1 = p1->accelTranslational();
...
Tddd uNeumann(const networkFace *const f_normal) {
   ...
   // double radius = Mean(extLength(fIN->getLines()));
   //
   // return optimizeNeumann(p, fIN, [&](const auto &f) {
   //    auto [p0, p1, p2] = f->getPoints();
   //    double radius = Mean(Tddd{p0->radius, p1->radius, p2->radius});
   //    if (f->getNetwork()->isRigidBody) {
   //       return std::tuple<Tddd, double>{
   //           f->getNetwork()->velocityRigidBody(fIN->center),
   //           w_Bspline5(Norm(Nearest(fIN->center, ToX(f)) - fIN->center), radius)};
   //    } else if (f->getNetwork()->isSoftBody) {
   //       auto v0 = p0->velocityTranslational();
   //       auto v1 = p1->velocityTranslational();
   //       auto v2 = p2->velocityTranslational();
   //       auto [t0, t1, X] = Nearest_(fIN->center, ToX(f));
   //       return std::tuple<Tddd, double>{v0 * t0 + v1 * t1 + v2 * (1 - t0 - t1),
   //                                       w_Bspline5(Norm(fIN->center - X), radius)};
   //    } else
   //       return std::tuple<Tddd, double>{{0., 0., 0.}, 0.};
   // });
};
Tddd accelNeumann(const networkFace *const f_normal) {
   ...
   //    if (f->getNetwork()->isRigidBody) {
   //       return std::tuple<Tddd, double>{
   //           f->getNetwork()->accelRigidBody(fIN->center),
   //           w_Bspline5(Norm(Nearest(fIN->center, ToX(f)) - fIN->center), radius)};
   //    } else if (f->getNetwork()->isSoftBody) {
   //       auto v0 = p0->accelTranslational();
   //       auto v1 = p1->accelTranslational();
