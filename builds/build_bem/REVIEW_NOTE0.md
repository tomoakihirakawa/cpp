# Contents
    - [⛵ BEM-MEL について](#-bem-mel-について)
        ...
        - [🪼 時間領域のBEM解析（ほとんどの場合BEM-MEL）](#-時間領域のbem解析ほとんどの場合bem-mel)
...
- [🐋 BEM-MEL について](#-bem-mel-について)
    - [⛵ 三角関数を使った古典的な解析手法](#-三角関数を使った古典的な解析手法)
    - [⛵ 周波数領域のBEM解析](#-周波数領域のbem解析)
        - [🪼 周波数領域BEM解析の計算フロー](#-周波数領域bem解析の計算フロー)
        - [🪼 BEM周波数領域の問題点](#-bem周波数領域の問題点)
            - [🪸 ポテンシャルの分離は妥当か？](#-ポテンシャルの分離は妥当か)
            - [🪸 報告例](#-報告例)
    - [⛵ 時間領域のBEM解析（ほとんどの場合BEM-MEL）](#-時間領域のbem解析ほとんどの場合bem-mel)
        - [🪼 BEM-MEL の問題点](#-bem-mel-の問題点)
    - [⛵ BEM-MEL の改良](#-bem-mel-の改良)
    - [⛵ 浮体動揺解析](#-浮体動揺解析)
        - [🪼 なぜ今BEM-MELを開発するのか](#-なぜ今bem-melを開発するのか)
            - [🪸 なぜFMMを使った流体-物体相互作用解析が難しいのか？](#-なぜfmmを使った流体-物体相互作用解析が難しいのか)
...
周波数領域解析は以下のような流れである（\cite{Kashiwagi2005}，\cite{Kashiwagi2004}を参考）
...
\cite{Feng2017}も指摘するように，以上のBEM周波数領域解析は，線型理論であるため波が激しい場合の精度には疑問が残る．
...
[./REVIEW_NOTE0_org.md#L1](./REVIEW_NOTE0_org.md#L1)
