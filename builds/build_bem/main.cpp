...
T6d velocity(const std::string &name,
			 const double start,
			 const double t)
{
	/* ------------------------------------------------------ */
...
VV_VarForOutput dataForOutput(const Network &water, const double dt)
{
	...
	uomap_P_Tddd P_accel_body, P_NearestContactFacesX,
		...
		P_uNeumann, P_normal, P_normal_BEM, P_mirrorPosition, P_U_normal_BEM,
		P_U_tangential_BEM, P_U_BEM, P_U_update_BEM, P_U_cling_to_Neumann;
	uomap_P_d P_IG, P_IGn, P_isGoodForQuad, P_smin_min, P_s_m, P_minViewRatio,
		...
		P_is_multiple_phiphin, P_min_depth, P_aphiat, P_aphiant, P_pressure, P_update_vs_cling, P_normalVariance;
	...
	P_accel_body = P_NearestContactFacesX = P_phin_Neumann = P_phin_Dirichlet = P_velocity_body = P_uNeumann = P_normal = P_normal_BEM = P_mirrorPosition = P_U_normal_BEM = P_U_tangential_BEM = P_U_BEM = P_U_update_BEM = P_U_cling_to_Neumann = initial_uomap_P_Tddd;
	P_IG = P_IGn = P_isGoodForQuad = P_smin_min = P_s_m = P_minViewRatio = P_volume = P_phi_Neumann = P_phi_Dirichlet = P_state = P_solidangleBIE = P_height = P_phi = P_phin = P_face_size = P_radius = P_lines_size = P_ishit = P_BC = P_Intxn_size = P_ContactFaces = P_is_multiple_phiphin = P_min_depth = P_aphiat = P_aphiant = P_pressure = P_update_vs_cling = P_normalVariance = initial_uomap_P_d;
...
int main()
{
	try
	{
		...
		for (time_step = 0; time_step < 1200; time_step++)
		{
			...
			{
				auto tmp = calculateForceFromPressure(water->getFaces(), net);
				std::cout << red << "force = " << tmp.force << ", area = " << tmp.area << reset << std::endl;
				jsonout.push(net->getName() + "_force", tmp.force);
				jsonout.push(net->getName() + "_torque", tmp.torque);
				jsonout.push(net->getName() + "_accel", tmp.acceleration);
			...
			mk_vtu(home_dir + "/test.vtu", FMM_BucketsFaces.getT4Tddd());
			// b@ ------------------------------------------------------ */
			...
			for (const auto &p : Points)
			{
				...
				p->RK_X.initialize(dt, real_time, p->getXtuple(), RK_order);
			}
			for (const auto &net : RigidBodyObject)
			{
					...
					if (net->inputJSON.find("velocity"))
					{
						...
						else if (move_name == "floating")
						{
							if (time_step > 0)
							{
								...
								auto force = tmp.getForce() + _GRAVITY3_ * net->mass;
								auto torque = tmp.getTorque(net->COM);
								...
								std::cout << red << net->getName() << "\n"
										  ...
										  << ", net->mass = " << net->mass << "\n"
										  << ", tmp.getForce() = " << tmp.getForce() << "\n"
										  << ", tmp.getTorque(net->COM) = " << tmp.getTorque(net->COM) << "\n"
										  << ", tmp.area = " << tmp.area << "\n"
						...
						{
							double start = stod(net->inputJSON["velocity"][1] /*start*/);
							std::cout << "start = " << start << std::endl;
							net->velocity = velocity(move_name, start, RK_time); // T6d //@ Φnを計算するために，物体表面の速度forced_velocityは，保存しておく必要がある
