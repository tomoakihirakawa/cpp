...
double accel_normal_from_Neumann_surface(const networkPoint *const p) {
   ...
   for (auto &[sF, cF] : getNearestContactFacesOfSurroundedNeumannFace(p) /*この点に隣接する流体面それぞれが最も近くで接している構造物の面*/) {
      A = sF->area;
      Atot += A;
      ret += A * Dot(cF->getNetwork()->accelRigidBody(p->getXtuple()), sF->normal);
      i++;
   }
...
double accel_normal_from_Neumann_surface(const networkPoint *const p, const networkFace *const f) {
   return Dot(closestFacingFace(f)->getNetwork()->accelRigidBody(p->getXtuple()), f->normal);
}
/* ------------------------------------------------------ */
double velocity_normal_from_Neumann_surface(const networkPoint *const p) {
   ...
   for (auto &[sF, cF] : getNearestContactFacesOfSurroundedNeumannFace(p) /*この点に隣接する流体面それぞれが最も近くで接している構造物の面*/) {
      A = sF->area;
      Atot += A;
      ret += A * Dot(cF->getNetwork()->velocityRigidBody(p->getXtuple()), sF->normal);
      i++;
   }
   return i == 0 ? ret : ret / Atot;
}
double velocity_normal_from_Neumann_surface(const networkPoint *const p, const networkFace *const f) {
   return Dot(closestFacingFace(f)->getNetwork()->velocityRigidBody(p->getXtuple()), f->normal);
}
/* ------------------------------------------------------ */
...
V_netLp getLinesAround(netPp p) {
   V_netLp ret = {};
   for (const auto &f : p->getFaces())
      for_each(f->getLinesTuple(), [&](const auto &l) { ret.emplace_back(l); });
...
#pragma omp single nowait
#endif
      {
         ...
         P_height[p] = std::get<2>(p->getXtuple());
      ...
      for (const auto &f : faces)
         if (f->Neumann) {
            ...
            if (isFinite(pressure012)) {
               this->PressureVeticies.emplace_back(std::tuple<Tddd, T3Tddd>{pressure012, T3Tddd{p0->getXtuple(), p1->getXtuple(), p2->getXtuple()}});
   ...
   try {
      ...
      //
      {
         ...
         for (auto i = 0; i < grid_refinement; i++) {
            ...
            for (auto ii = 0; ii <= loop; ++ii) {
               ...
               for (const auto &p : Points)
                  P_RK_X[p] = (new RungeKutta(dt, real_time, p->getXtuple(), RK_order));
      ...
      for (time_step = 0; time_step < 1200; time_step++) {
         ...
         for (const auto &net : RigidBodyObject) {
            ...
            if (net->inputJSON.find("velocity") && net->inputJSON["velocity"][0] == "floating") {
               auto tmp = calculateFroudeKrylovForce(water->getFaces(), net);
               uomap_P_Tddd P_COM, P_accel, P_velocity, P_rotational_velocity, P_rotational_accel, P_FroudeKrylovTorque;
               for (const auto &p : net->getPoints()) {
                  P_COM[p] = net->COM - p->getXtuple();
                  P_accel[p] = net->velocityRigidBody(p->getXtuple());
                  P_velocity[p] = net->accelRigidBody(p->getXtuple());
         ...
         }
         {
            ...
            for (const auto &f : water->getFaces()) {
               ...
               // 		break;
               // 	}
               for_each(f->getPoints(), [&](const auto &p) {if (p->CORNER){faces.emplace(f);} });
            }
