...
namespace forced_motion
{
	...
	T6d velocity(const double t)
	{
		...
		double T = 2.;
		return {0., 0., 0., sin(2. * M_PI / T * t), 0., 0.};
#else
		/* ------------------------------------------------------ */
...
Tddd getClosestStructureX(const networkPoint *const p)
{
	...
	{
		auto intxn = IntersectionSphereTriangle(p->getXtuple(), p->radius, f->getX_Vertices());
		if (intxn.isIntersecting && intxn.scale < 0 /*面の法線方向とは逆向き*/)
			tmpContactFaces[f] = intxn.X;
		else
		{
			auto X = intxn.getNearestX();
			auto norm = Norm(X - p->getXtuple());
			if (norm < p->radius && Dot(X - p->getXtuple(), f->getNormalTuple()) < 0.)
				tmpContactFaces[f] = X;
		}
	}
	Tddd ret = {1E+50, 1E+50, 1E+50};
	for (const auto &[f, X] : tmpContactFaces)
		if (Norm(ret) > Norm(X - p->getXtuple()))
			ret = X - p->getXtuple();
...
double phin_from_Neumann_surface(const networkPoint *const p)
{
	...
	for (auto &[sF, cF] : getNearestContactFacesOfSurroundedNeumannFace(p) /*この点に隣接する流体面それぞれが最も近くで接している構造物の面*/)
	{
		...
		A = sF->getArea();
		Atot += A;
		// ret += A * Dot(cF->getNetwork()->velocity, ToT6d(p->getNormalAreaAveraged()));
		// ret += A * Dot(cF->getNetwork()->velocity, ToT6d(sF->getNormalTuple()));
		/* ------------------------------------------------------ */
		// T6d VW = cF->getNetwork()->velocity;
		// Tddd CG = cF->getNetwork()->center_of_mass;
		// Tddd Vb = {std::get<0>(VW), std::get<1>(VW), std::get<2>(VW)};
		// Tddd Wb = {std::get<3>(VW), std::get<4>(VW), std::get<5>(VW)};
		// ret += A * Dot(Vb + Cross(Wb, p->getXtuple() - CG), sF->getNormalTuple());
		/* ------------------------------------------------------ */
...
#define Dombre2019
				...
				*/
				...
				p->U_tangential_BEM = p->U_BEM - Dot(p->U_BEM, p->getNormalAreaAveraged()) * n;
...
#define area_averaged_tension
			...
			if (p->CORNER)
			{
				...
				if (p->getSolidAngle() < 4. * M_PI / 4.)
					tension *= std::pow(p->getSolidAngle() / (4. * M_PI / 4.), 2.);
				if (p->getSolidAngle() > 4. * M_PI * 3. / 4.)
					tension *= std::pow((4. * M_PI - p->getSolidAngle()) / (4. * M_PI / 4.), 2.);
			}
			...
			if ((p->Neumann || p->CORNER) && !p->getContactFaces().empty())
			{
				...
				// local_velocity_from_Neumann_surface()

				// 2022/01/15以前は，以下のようにして，接している面方向には進めないようにしていた．
				//  for (const auto &f : p->getContactFaces())
				//  {
				//  	auto n = f->getNormalTuple();
				//  	p->U_BEM -= Dot(p->U_BEM, n) * n;
				...
				for (const auto &[f, X] : tmpContactFaces)
				{
					...
					//
					p->U_update_BEM -= Dot(p->U_update_BEM, n) * n;
					p->U_update_BEM += Dot(V, n) * n;
				}

				// if (!p->getContactFaces().empty())
				// {
				// 	auto V = local_velocity_from_Neumann_surface(p);
				// 	auto n = Normalize(V);
...
int main()
{
	try
	{
		...
		for (time_step = 0; time_step < 10000; time_step++)
		{
			...
			double min_dt = 1E+10;
			for (const auto &p : water.getPoints())
				for (const auto &q : TakeExcept(Flatten(BFS(p, 5)), p))
				{
					auto dir = (q->getXtuple() - p->getXtuple());
					auto vel_to_q = std::abs(Dot(p->U_BEM, Normalize(dir)));
			...
			// b* ------------------------- 出力 ------------------------- */
			{
				...
				uomap_P_Tddd P_accel_body, P_closestStructureX;
				...
				try
				{
					for (const auto &p : water.getPoints())
					{
						...
						P_closestStructureX[p] = getClosestStructureX(p);
				...
				try
				{
					...
					VV_VarForOutput data = {
						...
						{"closestStructureX", P_closestStructureX},
						// P_closestStructureXで確かに最寄の構造物までをさすことができている．！
						// これで改善できない？
