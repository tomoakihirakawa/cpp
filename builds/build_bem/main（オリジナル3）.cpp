...
void calculateVectorToSurfaceInBuffer(const Network &net)
{
	/*
...
#pragma omp single nowait
			...
			if (p->CORNER)
			{
				...
				//% ------------------------------------------------------ */
				{
					...
					{
						auto [p0, p1, p2] = f->getPointsTuple();
						auto X0 = p0->RK_X.getX(f->getNetwork()->velocityRigidBody(p0->getXtuple()));
						auto X1 = p1->RK_X.getX(f->getNetwork()->velocityRigidBody(p1->getXtuple()));
						auto X2 = p2->RK_X.getX(f->getNetwork()->velocityRigidBody(p2->getXtuple()));
...
void calculateVectorToClingNeumann(const Network &net)
{
	/*
	@ ノイマン面に貼り付けるための必要な調整
	*/
...
#pragma omp single nowait
		...
		*/
		for (const auto &p : Points)
			p->U_BUFFER = p->U_BUFFER_BUFFER = {0., 0., 0.};
		if (adjust_mesh)
			calculateVectorToSurfaceInBuffer(net);
		/* ------------------------------------------------------ */
		//@ ノイマン面に張り付くようにN，Cでの節点のU_BUFFERを修正
		calculateVectorToClingNeumann(net); //@必須
		for (const auto &p : Points)
		...
		for (time_step = 0; time_step < 2000; time_step++)
		{
			...
			do
			{
				...
				for (const auto &net : RigidBodyObject)
				{
					...
					/* ------------------------- */
					for (const auto &p : net->getPoints())
						p->RK_X.push(net->velocityRigidBody(p->getXtuple())); //上書きされる．しかし，calculateVectorToSurfaceInBufferなどのためにこのプッシュをしておく必要がある
					//
