...
effect(const float* imgData, const int width, const int height, const int ch)
{
    ...
    const int filtersize = (int)sqrt((double)(sizeof(filter)/sizeof(filter[0])));
        ...
        for (x = filtersize / 2; x < width - (filtersize / 2); x++)
        {
            ...
            for (int fy = 0; fy < filtersize; fy++)
            {
                ...
                for (int fx = 0; fx < filtersize; fx++)
                {
                    ...
                    blue  += filter[fy*filtersize+fx] * imgData[iy*step + ix * ch + 0];
                    green += filter[fy*filtersize+fx] * imgData[iy*step + ix * ch + 1];
                    red   += filter[fy*filtersize+fx] * imgData[iy*step + ix * ch + 2];
                }
            }
