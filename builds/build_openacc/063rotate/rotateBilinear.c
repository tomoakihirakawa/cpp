...
// rotate
float*
effect(const float* in, const int width, const int height, const float degree)
{
    unsigned int datasize = sizeof(float)*width*height;
    ...
    //float radian = (float)degree2radian(360.0f - degree);// counter clockwise
    ...
    #pragma acc data copyin(in[:width*height]) copyout(out[:width*height])
    {
    #pragma acc kernels
    #pragma acc loop independent
    for (outY = -yc; outY < height - yc; outY++)    // dest y coord
    {
        #pragma acc loop independent
...
main(int argc, char* argv[])
{
    ...
    float* out = effect(in, width, height, degree);
    if (out == NULL)
    {
        free(in);
