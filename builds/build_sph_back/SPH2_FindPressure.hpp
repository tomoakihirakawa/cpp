...
#pragma omp single nowait
      {
         ...
         */
         ...
         //    auto grad = grad_w_Bspline(X_next(A) + A->v_to_surface_SPH, X_next(B), A->SML_next());
         ...
         auto EISPH_wall_pressure = [&ROW, &pO, &pO_center, &pO_center_mirror, &total_weight](const auto &B /*column id*/) {
            ...
            if (Distance(pO_center, BX) < r) {
               ...
               total_weight += w;
               auto dP = Dot(pO_center_mirror - BX, B->mu_SPH * B->lap_U_next + rho_next(B) * _GRAVITY3_);
               ROW->p_EISPH += (B->p_SPH_last + dP) * w;
         ...
         auto PoissonEquation = [&ROW, &pO, &pO_center, &sum_Aij_Pj, &sum_Aij, &applyOverPoints, &target_nets](const auto &B /*column id*/) {
            const auto BX = X_next(B);
            const auto r = pO->SML_next();
            if (Distance(pO_center, BX) < r) {
               ...
               //! 修正
               // Aij *= -1;
               const auto rij = (pO_center - BX);
               applyOverPoints([&](const auto &Q) {
                  auto grad = grad_w_Bspline_next({pO, pO_center}, Q);
                  auto c = Aij * Dot(rij, Q->volume * grad);
                  ROW->increment(pO, c);
                  ROW->increment(Q, -c);
               },
                               target_nets);
